...
function _wap_assign_weixinsdk()
{
    global $smarty,$db,$ecs;
    $is_weixin = is_weixin();
	if($is_weixin){
		$weixin_info = $db->getRow("SELECT appid,appsecret FROM ".$ecs->table('weixin_config'));
		if($weixin_info['appid'] && $weixin_info['appsecret']){
			$smarty->assign('is_weixin', '1');
			require_once $_SERVER['DOCUMENT_ROOT']."/mobile/wxjs/jssdk.php";
