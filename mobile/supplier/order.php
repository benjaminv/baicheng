...
elseif($act == 'info')
{
	...
	//    }
	if($order['district']){
		$order['address'] = get_region_info($order['district']).$order['address'];
	}
	if($order['city']){
		$order['address'] = get_region_info($order['city']).$order['address'];
	}
	if($order['province']){
		$order['address'] = get_region_info($order['province']).$order['address'];
	}
    /* 取得上一个、下一个订单号 */
...
elseif($act == 'edit')
{
    ...
    /* 取得参数 step */
    $step_list = array('user', 'goods', 'consignee', 'shipping', 'payment', 'other', 'money','edit_goods');
	...
	elseif('edit_goods' == $step){
		//获取订单详情
		$order_goods = $db->getAll("SELECT rec_id,goods_id,goods_number,goods_price,goods_name,goods_attr FROM ".$ecs->table('order_goods')." WHERE order_id=".$order_id);
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        if ($payment['is_cod'])
        {
            ...
            $arr['shipping_time_end']=gmtime();
            $order['shipping_time_end']=gmtime();
        }
        update_order($order_id, $arr);
    ...
    elseif ('receive' == $operation)
    {
        ...
        $arr = array('shipping_status' => SS_RECEIVED,'shipping_time_end'=>gmtime());
        $payment = payment_info($order['pay_id']);
        if ($payment['is_cod'])
        {
            $arr['pay_status'] = PS_PAYED;
...
//商家卡项项目核销
elseif($_REQUEST['act'] == 'column_sign'){
	//获取商家可以核销的列表
	$data = column_list();
	foreach($data['list'] as $k=>$v){
		$data['list'][$k]['add_time'] = local_date('Y-m-d H:i',$v['add_time']);
	}
    $smarty->assign('list',$data['list']);
    $smarty->assign('filter',$data['filter']);
	$smarty->assign('order_id',isset($_REQUEST['order_id'])?intval($_REQUEST['order_id']):0);
	_wap_assign_weixinsdk();//加载微信jssdk
    _wap_assign_header_info('卡项核销','',0,0,0,0);
    _wap_assign_footer_order_info();
    _wap_display_page('column_sign.htm');
}

elseif($_REQUEST['act'] == 'ajax_column'){
	$id = intval($_POST['id']);
	if(empty($id)){
		echo json_encode(array('code'=>0,'msg'=>'卡项编号出错!'));exit;
	}
	$info = column_info($id);
	echo json_encode(array('code'=>1,'list'=>$info));
}

elseif($_REQUEST['act'] == 'item_verify'){
	$column_use = $_POST['column_use'];
	unset($_POST);
	if(empty($column_use)){
		echo json_encode(array('code'=>0,'msg'=>'卡项核销出错!'));exit;
	}
	//过滤掉数量不合法的核销项目
	foreach($column_use as $k=>$v){
		if(intval($v)<=0){
			continue;
		}else{
			$column_info = $GLOBALS['db']->getRow('SELECT a.id,a.column_name,(a.column_number-a.column_use_number) as column_left_number FROM '.$GLOBALS['ecs']->table('card_item_goods').' a LEFT JOIN '.$GLOBALS['ecs']->table('order_info').' b ON a.order_id=b.order_id WHERE a.id='.$k.' AND a.supplier_id='.$_SESSION['supplier_id'].' AND b.pay_status > 0 ');
			if($column_info['column_left_number']<intval($v)){
				echo json_encode(array('code'=>0,'msg'=>$column_info['column_name'].'的核销数量不合法!'));exit;
			}
		}
	}
	...
	foreach($column_use as $k=>$v){
		if(intval($v)<=0){
			...
			$column_info =  $GLOBALS['db']->getRow('SELECT column_id,(column_number-column_use_number) as column_left_number FROM '.$GLOBALS['ecs']->table('card_item_goods').'  WHERE id='.$k);
			//修改用户项目可核销次数
			$sql = "UPDATE ".$GLOBALS['ecs']->table('card_item_goods'). " SET column_use_number = column_use_number + ".intval($v)." WHERE id = " . $k;
			...
			$data = array(
				...
				'column_number'	=>	intval($v),
				'left_number'	=>	$column_info['column_left_number']-intval($v),
			);
			$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('supplier_card_item_log'), $data, 'INSERT', '', 'SILENT');
		}
	}
...
function column_info($id){
	$order_info = $GLOBALS['db']->getRow("SELECT a.user_name FROM ".$GLOBALS['ecs']->table('users')." a LEFT JOIN ".$GLOBALS['ecs']->table('order_info')." b ON a.user_id=b.user_id WHERE b.order_id=".$id);
	$order_info['column_info'] = $GLOBALS['db']->getAll('SELECT a.id,a.column_name,a.column_number,a.column_use_number,(a.column_number-a.column_use_number) as column_left_number,c.user_name,b.order_sn FROM '.$GLOBALS['ecs']->table('card_item_goods').' a LEFT JOIN '.$GLOBALS['ecs']->table('order_info').' b ON a.order_id=b.order_id LEFT JOIN '.$GLOBALS['ecs']->table('users').' c ON b.user_id=c.user_id WHERE a.order_id='.$id.' AND a.supplier_id='.$_SESSION['supplier_id'].' AND b.pay_status > 0 ');
	return $order_info;
}
/**
 *  获取订单列表信息  自身店铺的订单
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = "WHERE ((o.supplier_id='".$_SESSION['supplier_id']."') OR (d.supplier_id='".$_SESSION['supplier_id']."')) AND o.extension_code <> 'virtual_good' ";
		...
		$sql = "SELECT COUNT(DISTINCT o.order_id) ".
				...
				" LEFT JOIN " .$GLOBALS['ecs']->table('order_goods'). " a ON o.order_id=a.order_id".
				" LEFT JOIN " .$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('card_item')." c ON b.item_id=c.id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('supplier_circle_list')." d ON c.circle_id=d.circle_id ".
				$where;

        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
		//edit by yhy 自己店铺的订单和卡项的订单
        /* 查询 */
        $sql = "SELECT DISTINCT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,b.item_id,e.supplier_name ".
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('order_goods'). " a ON o.order_id=a.order_id".
				" LEFT JOIN " .$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('card_item')." c ON b.item_id=c.id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('supplier_circle_list')." d ON c.circle_id=d.circle_id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('supplier')." e ON o.supplier_id=e.supplier_id ".
				$where .
...
function column_list(){
    ...
    if ($result === false)
    {
        ...
        $where = "WHERE a.supplier_id='".$_SESSION['supplier_id']."'";
		...
		$sql = "SELECT COUNT(a.id) ".
                " FROM " . $GLOBALS['ecs']->table('supplier_card_item_log') . " a " .
				$where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        /* 查询 */
        $sql = "SELECT a.*,b.column_name,d.user_name,c.order_id,c.order_sn ".
                " FROM " . $GLOBALS['ecs']->table('supplier_card_item_log') . " AS a " .
                " LEFT JOIN " .$GLOBALS['ecs']->table('card_item_goods'). " AS b ON a.item_goods_id=b.id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('order_info'). " AS c ON b.order_id=c.order_id ".
				" LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS d ON c.user_id=d.user_id ".
				$where .
                " ORDER BY $filter[sort_by] $filter[sort_order] ".
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
        set_filter($filter, $sql);
