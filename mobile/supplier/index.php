...
if ($_REQUEST['act'] == '')
{
	...
	$is_ipc_shopping = $db->GetOne('SELECT is_ipc_shopping FROM ' .$ecs->table('supplier'). ' WHERE  supplier_id = '.$_SESSION['supplier_id']);
	$smarty->assign('is_ipc_shopping',$is_ipc_shopping);
	//店铺是否参加联盟卡项
	$card_item = $db->GetOne("SELECT COUNT(id) FROM ".$ecs->table('card_item_column')." WHERE supplier_id=".$_SESSION['supplier_id']);
	$card_item_goods = $db->GetOne("SELECT COUNT(a.id) FROM ".$ecs->table('card_item_goods')." a LEFT JOIN ".$ecs->table('order_info')." b ON a.order_id=b.order_id WHERE b.pay_status>0 AND a.supplier_id=".$_SESSION['supplier_id']);
	$smarty->assign('card_item',$card_item);
	$smarty->assign('card_item',$card_item_goods);
