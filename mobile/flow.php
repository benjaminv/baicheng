...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'selcart')
{
	...
	if ($_GET['sel_goods'])
	{
		...
		*/
		...
		/*勾选购物车后，清除使用的积分session edit by yhy 2019/7/8*/
		$_SESSION['flow_order']['integral_info'] = "";
		$_SESSION['flow_order']['integral'] = 0;
		unset($_SESSION['flow_order']['integral_info']);
		unset($_SESSION['flow_order']['integral']);
		/*end*/
	}
	else
	{
...
if ($_REQUEST['step'] == 'add_to_cart')
{
   ...
   include_once('includes/cls_json.php');
    ...
    //判断是否有权限
    if($_REQUEST['act']=='check_goods'){
        $goods_level=$GLOBALS['db']->getOne("SELECT `level` FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id='".$_REQUEST['goods_id']."'");

        $user_rank = $GLOBALS['db']->getOne("SELECT user_rank FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id='".$_SESSION['user_id']."'");
        if(!in_array($user_rank,explode(',',$goods_level))&&$goods_level!=0){
            $result= -1;
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
			...
			/*购物车变动后，清除使用的积分session edit by yhy 2019/7/8*/
				$_SESSION['flow_order']['integral_info'] = "";
				$_SESSION['flow_order']['integral'] = 0;
				unset($_SESSION['flow_order']['integral_info']);
				unset($_SESSION['flow_order']['integral']);
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $smarty->assign('city_list',     $city_list);
        $smarty->assign('district_list', $district_list);
		$schedul_need = exist_technican_goods();
		$shipping_need = exist_real_goods(0, $flow_type);
		if($schedul_need['code'] == 1 || $shipping_need){
			/* 返回收货人页面代码 */
			$smarty->assign('real_goods_count',  1);
		}else{
			 /* 返回收货人页面代码 */
			$smarty->assign('real_goods_count',  0);
		}
    }
    else
    {
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	$default_supplier_id = get_user_default_supplier(intval($_SESSION['user_id']));
    /*------------------------------------------------------ */
    //-- 订单确认
    /*------------------------------------------------------ */
	...
	//获取店铺的技师列表 edit yhy 2019/5/31
		$exist_service_goods = 0;
		//传入第三个参数查询是否存在平台的服务，存在的话，需要查询技师
		if(exist_service_goods(0, $flow_type,0)){
			$technican_list = get_technican_list($default_supplier_id);
			$smarty->assign('technican_list', $technican_list);
			$exist_service_goods = 1;
		}
		$smarty->assign('exist_service_goods', $exist_service_goods);
	//end
	

    /* 检查收货人信息是否完整 并且订单中存在非服务类商品*/
	//是否需要收件人以及配送方式
	$shipping_need = exist_real_goods(0, $flow_type);
	$smarty->assign('shipping_need', $shipping_need);
	$schedul_need = exist_technican_goods();
	$smarty->assign('schedul_need', $schedul_need['code']);
    if (!check_consignee_info($consignee, $flow_type) && ($shipping_need || $schedul_need['code']==1))
    {
         /* 如果不完整则转向到收货人信息填写界面 */
         ecs_header("Location: flow.php?step=consignee\n"); 
         exit;
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    ...
    if(count($cart_goods)>0){
    	foreach($cart_goods as $key => $val){
			...
			$cart_goods_new[$val['supplier_id']]['rec_ids'][]=$val['rec_id'];
    	}
		$schedul_list = array();
		foreach($cart_goods_new as $key=>$val){
			$cart_goods_new[$key]['shipping_need'] = exist_real_goods(0, $flow_type,$key,implode(',',$val['rec_ids']));
			if($cart_goods_new[$key]['shipping_need']){
				$cart_goods_new[$key]['shipping_html'] = insert_get_shop_shipping(array('suppid'=>$key,'consignee'=>$consignee,'flow_type'=>$flow_type));
			}	
			//初始化店铺可使用积分为0
			$cart_goods_new[$key]['integral_can_use'] = 0;
			//查询是否存在技师店铺的服务，存在的话，给出技师行程 start yhy 
			
			$schedul_result = exist_technican_goods($flow_type,$key);
			if($schedul_result['code'] == 1){
				$cart_goods_new[$key]['exist_technican_service_goods'] =1;
				$schedul_list[$key] = $schedul_result['schedul_list'];
			}else{
				$cart_goods_new[$key]['exist_technican_service_goods'] =0;
			}
		}
		$smarty->assign('schedul_list',json_encode($schedul_list));
    }
    ...
    {
        $smarty->assign('allow_edit_cart', 1);// zouzheli 
    }
	...
	$_CFG['use_integral'] = '1';//先写死积分可用开关已打开
    if ((!isset($_CFG['use_integral']) || $_CFG['use_integral'] == '1')
        && $_SESSION['user_id'] > 0
        && $user_info['pay_points'] > 0
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
		//判断每个店铺的订单中是否存在可使用积分的订单，并且积分最多使用金额
		$integral_can_use = flow_available_points();// 可用积分
        foreach($integral_can_use as $k=>$v){
        	$cart_goods_new[$k]['integral_can_use'] = $v;
        }
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    /* 对商品信息赋值 */
    $cart_goods = cart_goods($flow_type); // 取得商品列表，计算合计
    //if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))  //2c不需要收货地址，去掉判断收货地址是否完整
    //{
	if (empty($cart_goods))
    {
        $result['error'] = $_LANG['no_goods_in_cart'];
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	/*edit by yhy start 是否选择技师*/
	$exist_service_goods = intval($_POST['exist_service_goods']);
	if($exist_service_goods){
		$technican_info = $db->getRow("select technican_name,technican_sn FROM ". $GLOBALS['ecs']->table('technican') . " WHERE id=".intval($_POST['technican']));
		if(!isset($technican_info) || empty($technican_info)){
			show_message("请选择技师");exit;
		}
	}
    ...
    $point_case = array();
    /*采用积分抵扣商品金额*/
    if (isset($_POST['ECS_USE_POINTS']) && !empty($_POST['ECS_USE_POINTS'])) {
        require(ROOT_PATH . '../includes/lib_points.php');
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
			...
			'from_supplier_id'=>	$default_supplier_id
	        );
		$order['defaultbank'] = $_POST['www_68ecshop_com_bank'] ? trim($_POST['www_68ecshop_com_bank']) : "";
		if(isset($_POST['schedul_datetime'][$ckey]) && $_POST['schedul_datetime'][$ckey] == 1 && (empty($_POST['schedul_'][$ckey]) || empty($_POST['schedul_day_'][$ckey]))){
			show_message('请选择服务时间');
		}else{
			$order['schedul_date'] = trim($_POST['schedul_day_'][$ckey]);
			$order['schedul_time'] = trim($_POST['schedul_'][$ckey]);
		}
		...
		$exist_real_goods = exist_real_goods(0,$flow_type,$ckey,implode(',',$cval['rec_ids']));
    ...
    foreach($order_info as $ok=>$order){
		...
		$send=false;
	    /* 如果订单金额为0（使用余额或积分或红包支付）并且不存在实体商品，修改订单状态为已确认、已付款 */
		...
		$order['technican_name'] = isset($technican_info['technican_name'])?$technican_info['technican_name']:"";
		$order['technican_sn'] = isset($technican_info['technican_sn'])?$technican_info['technican_sn']:"";
	    ...
	    $fields = "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, ".
            "goods_price,split_money,sale_point,level_point,supplier_split_money,supplier_level_fuwufe, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id, use_points_by_one, deduction_amount_by_one, system_point_exchange";

	    $sql_insert_order_goods = "INSERT INTO " . $ecs->table('order_goods') . " ( " . $fields . ") ";

	    $sql_query_cart_goods = " SELECT '$new_order_id' as 'order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, goods_price,split_money,sale_point,level_point,supplier_split_money,supplier_level_fuwufe, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id,0 as use_points_by_one,0 as deduction_amount_by_one,0 as system_point_exchange".
	            " FROM " .$ecs->table('cart') .
        ...
        foreach ($cart_goods as $v) {
            if ($v['goods_id'] == $temp_cart_goods['goods_id']) {
                ...
                $sql_query_cart_goods = " SELECT '$new_order_id' as 'order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, goods_price,split_money,sale_point,level_point,supplier_split_money,supplier_level_fuwufe, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id, {$use_points_by_one} as use_points_by_one, {$deduction_amount_by_one} as deduction_amount_by_one , {$system_point_exchange} as system_point_exchange".
                    " FROM " .$ecs->table('cart') .
                    " WHERE $sql_where AND rec_type = '$flow_type' $id_ext_new ";
                break;
            }
		...
		//如果订单为卡项订单，新增卡项订单关联表数据 2019/8/2 edit yhy
		card_item_order_goods($new_order_id);
		...
		$split_order['suborder_list'][$ok]['shipping_name'] = isset($order['shipping_name'])?$order['shipping_name']:'';
	    ...
	    if ($order['order_amount'] <= 0){
			/* 如果订单金额为0（使用余额或积分或红包支付），自动分成*/
				include_once(ROOT_PATH . 'includes/lib_pay_fencheng.php');
				get_pay_fencheng($new_order_id);
				log_supplier_commission($new_order_id);
				/*判断技师店铺订单，并且预约了时间*/
				schedul_change($new_order_id);
			 $send=false;
			//卡项订单状态变为已发货,并且发放积分
			if(is_card_item_order($new_order_id)){
				$sql = 'update ecs_order_info set order_status=5,shipping_status=1,pay_status=2,is_separate=0 where order_id='.$new_order_id;
				$GLOBALS['db']->query($sql);

				//发货后发放积分
				$order_info = $GLOBALS['db']->getRow("SELECT user_id,order_sn,extension_id,extension_code,extension_id,goods_amount,order_id FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id=".$new_order_id);
				$integral = integral_to_give($order);
				log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), '订单:'.$order['order_sn'].'赠送的积分');
			}


			/*不存在实体商品的订单订单状态变为已收货,并且发放分销和店铺分成*/
			
			 //if(!exist_real_goods(0,$flow_type,$ok,implode(',',array_keys($cart_goods)))){
				//$sql = 'update ecs_order_info set order_status=5,shipping_status=0,pay_status=2,is_separate=0 where order_id='.$new_order_id;
				//$GLOBALS['db']->query($sql);
				//判断为技师店铺的话，订单状态为已发货状态
				//$is_ipc_shopping = $GLOBALS['db']->getOne("SELECT is_ipc_shopping FROM ".$GLOBALS['ecs']->table('supplier')." WHERE supplier_id=".$ok);
				//if($is_ipc_shopping == 2){
					//$sql = 'update ecs_order_info set order_status=5,shipping_status=0,pay_status=2,is_separate=0 where order_id='.$new_order_id;
					//$GLOBALS['db']->query($sql);
				//}else{
					//$sql = 'update ecs_order_info set order_status=5,shipping_status=0,pay_status=2,is_separate=1 where order_id='.$new_order_id;
					//$GLOBALS['db']->query($sql);
					//distrib_separate($new_order_id);
					//supplier_separate_by_orderid($new_order_id);
				//}
				////发货后发放积分
				//$order_info = $GLOBALS['db']->getRow("SELECT user_id,order_sn,extension_id,extension_code,extension_id,goods_amount,order_id FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id=".$new_order_id);
				//$integral = integral_to_give($order);
			...
			/*订单加入同步队列*/
			if(ODOO_BC_ERP){
				putOrderIntoQueue($order['order_sn']);
			}
   ...
   if($send){
	...
	if ($order['order_amount'] > 0)
    {
       ...
       if ($payment['pay_code']=='weixin'||$payment['pay_code']=='alipay'){
			$pay_online = array('online'=>1);
			//$pay_obj->get_code($order, unserialize_config($payment['pay_config']));
	   }else{
     ...
     //判断，当支付成功后跳转到分享页面
      //if($order['order_amount']<=0){
          //ecs_header("Location: fx.php?u=".$_SESSION['user_id']."&oid=".$order['order_id']."\n");
     //}
...
elseif ($_REQUEST['step'] == 'drop_goods')
{
	...
	/*移除购物车后，清除使用的积分session edit by yhy 2019/7/8*/
		$_SESSION['flow_order']['integral_info'] = "";
		$_SESSION['flow_order']['integral'] = 0;
		unset($_SESSION['flow_order']['integral_info']);
		unset($_SESSION['flow_order']['integral']);
...
if($_REQUEST['step']=='update_group_cart')
{
	...
	/*购物车数量变更后，清除使用的积分session edit by yhy 2019/7/8*/
		$_SESSION['flow_order']['integral_info'] = "";
		$_SESSION['flow_order']['integral'] = 0;
		unset($_SESSION['flow_order']['integral_info']);
		unset($_SESSION['flow_order']['integral']);
...
function flow_available_points()
{
	...
	//if($supplier_id>0){
		//$sql_where .= " AND c.supplier_id=".$supplier_id;
	//}
	if ($_SESSION['sel_cartgoods']){
		$sql_where .= " AND c.rec_id in (". $_SESSION['sel_cartgoods'] .") ";
	}
