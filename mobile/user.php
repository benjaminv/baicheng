...
if(empty($_SESSION['user_id']))
{
	if(! in_array($action, $not_login_arr))
	{
		if(in_array($action, $ui_arr))
		{
			...
			//$portal_id=$GLOBALS['db']->getOne("select portal_id from ".$GLOBALS['ecs']->table('users')." where user_id=".$up_uid);
			//header("location: ".OSS_LOGIN."/?uid=".$portal_id."&dest=".$back_act);exit;
		}else{
			// 未登录提交数据。非正常途径提交数据！
...
function action_comment_edit(){
    ...
    $comment_id=empty($_REQUEST['comment_id'])?0:intval($_REQUEST['comment_id']);
    $min_editdays = gmtime() - 86400 * $_CFG['comment_editdays'];
    $res_comment=$db->getRow("SELECT * FROM " . $ecs->table('comment') . " WHERE comment_id = '$comment_id' AND user_id = '$user_id'");
    if(!$res_comment){
        echo "<script>alert('此订单商品评价不存在，无法处理！');location='user.php?act=my_comment';</script>";
        exit();
	}
    if($res_comment&&($res_comment['is_edit']==1||($res_comment['add_time']<$min_editdays))){
        echo "<script>alert('此订单商品评价无法修改了！');location='user.php?act=my_comment';</script>";
        exit();
    }
    //print_r($res_comment);die();
    $goods = get_goods_info_wap($res_comment['id_value']);
    $smarty -> assign('goods',$goods);
    $smarty -> assign('comment',$res_comment);
    $smarty -> display('comment_edit.dwt');
}

//评价追加
function action_comment_added(){
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    $smarty = $GLOBALS['smarty'];
    ...
    $comment_id=empty($_REQUEST['comment_id'])?0:intval($_REQUEST['comment_id']);
    $min_zhuijiadays = gmtime() - 86400 * $_CFG['comment_zhuijiadays'];
    $res_comment=$db->getRow("SELECT * FROM " . $ecs->table('comment') . " WHERE comment_id = '$comment_id' AND user_id = '$user_id'");
    if(!$res_comment){
        echo "<script>alert('此订单商品评价不存在，无法处理！');location='user.php?act=my_comment';</script>";
        exit();
    }
    $comment_added=$db->getRow("SELECT * FROM ".$ecs->table('comment_added')." WHERE comment_id=".$comment_id);
    if($comment_added){
        echo "<script>alert('已追加过评价了，无法再处理！');location='user.php?act=my_comment';</script>";
        exit();
    }
    if($res_comment['add_time']<$min_zhuijiadays){
        echo "<script>alert('无法追加评价！');location='user.php?act=my_comment';</script>";
        exit();
    }
    $goods = get_goods_info_wap($res_comment['id_value']);
    $smarty -> assign('goods',$goods);
    $smarty -> assign('comment',$res_comment);
    $smarty -> display('comment_added.dwt');
}

//评价追加保存
function action_comment_added_save(){
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    $smarty = $GLOBALS['smarty'];
    ...
    $ip_address = real_ip();
    $db->query("INSERT INTO " . $ecs->table('comment_added') . "(comment_id,content,add_time,ip_address,status) VALUES ('".$comment_id."','".$content."','".gmtime()."','".$ip_address."','".$status."')");
    clear_cache_files();
    if($status == 0)
    {
...
function action_login()
{
	...
	if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')){
		$wx_login ="weixin_login.php?user_id=0&erweima_type=0&callback=".$back_act;
		header("location: ".$wx_login);exit;
	}
...
//		), 'info');
		...
		show_message($_LANG['login_success'] . $ucdata, $_LANG['profile_lnk']
		, 'user.php'
		, 'info');
	}
	else
...
//		$back_act,'index.php'
//	), 'info');
	show_message($_LANG['logout'] . $ucdata, $_LANG['back_home_lnk']
		,'index.php'
	, 'info');
}
...
function action_back_order_detail_edit ()
{
	...
	$db->query($sql);
		...
		if(ODOO_ERP){
			//edit yhy 更新退货单的物流单号
			$res = $GLOBALS['odooErpObj']->syncRefundOrderByOrdersn($back_id,1,1);
		}
...
function action_back_order_act ()
{
	...
	{//退货退款
		// 找出退货商品抵扣的积分
		$sql = "SELECT * FROM " . $ecs->table('order_goods') . " WHERE order_id ='" . $_POST['order_id'] ."' AND goods_id = '" . $_POST['goods_id'] ."'";
		$tui_goods_back_type1_info = $db->getRow($sql);

		$have_tuikuan = 1;
		$tui_goods_number = $_REQUEST['tui_goods_number'] ? intval($_REQUEST['tui_goods_number']) : 1;
		$sql = "insert into " . $ecs->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, " . "back_goods_number, back_goods_price, status_back, use_points_by_one, deduction_amount_by_one ) " . " values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '0', " . " '$tui_goods_number', '$_REQUEST[tui_goods_price]', '5',". "'" . $tui_goods_back_type1_info['use_points_by_one'] ."'".  ",'" . $tui_goods_back_type1_info['deduction_amount_by_one']."') ";
		$db->query($sql);
	...
	//if(ODOO_ERP){
			////edit yhy
			//$res = $GLOBALS['odooErpObj']->syncRefundOrderByOrdersn($back_id,0,$back_type);
	//}
		...
		if ($_POST['order_all'])
		{
			...
			if($order_info['shipping_fee']>0){
				if($order_info['money_paid']>0){
					$order_info['shipping_fee'] = min($order_info['money_paid'],$order_info['shipping_fee']);
					$order_info['money_paid'] -= $shipping_fee;
				}
				if($order_info['shipping_fee']>0){
					$order_info['shipping_fee'] = min($order_info['surplus'],$order_info['shipping_fee']);
					$order_info['surplus'] -= $order_info['shipping_fee'];
				}
			}
			////整单申请退款
			$sql = "update " . $ecs->table('back_order') . " set refund_money_1= ".$order_info['money_paid'].",refund_money_2=".$order_info['surplus'].",refund_money_3=".$order_info['integral']." where back_id='$back_id' ";
			$db->query($sql);
		}
		else
		{
			//部分退款退货,获取退款金额。
			$refund_money = getRefundMoney($orderinfo,$back_id);
			$sql = "update " . $ecs->table('back_order') . " set refund_money_1= ".$refund_money['money_paid'].",refund_money_2=".$refund_money['surplus'].",refund_money_3=".$refund_money['integral']." where back_id='$back_id' ";
			$db->query($sql);
		}
	}
	else
...
//获取退货退款单的退款金额
function getRefundMoney($orderinfo,$back_id){
	//获取订单已经退款的金额
	$back_money = $GLOBALS['db']->getRow("SELECT SUM(`refund_money_1`) as money_paid,SUM(`refund_money_2`) as surplus,SUM(`refund_money_3`) as integral,SUM(`shipping_fee`) as shipping_fee FROM ".$GLOBALS['ecs']->table('back_order')." WHERE order_id=".$orderinfo['order_id']);
	
	if($back_money['money_paid']){
		$orderinfo['money_paid'] -= $back_money['money_paid'];
	}
	if($back_money['surplus']){
		$orderinfo['surplus'] -= $back_money['surplus'];
	}
	if($back_money['integral']){
		$orderinfo['integral'] -= $back_money['integral'];
	}
	$surplus = $integral =0;

	//查询退款退货单总共可以使用的积分数量
	$back_goods = $GLOBALS['db']->getAll("SELECT b.integral as integral_money,a.back_goods_number,a.back_goods_price FROM ".$GLOBALS['ecs']->table('back_goods')." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE a.back_id=".$back_id);

	$money_paid = $back_goods['back_goods_price']*$back_goods['back_goods_number']; 
	if($orderinfo['integral']>0){//订单是否有积分可以退
		$scale = floatval($GLOBALS['_CFG']['integral_scale']);
		if($scale>0){
			$integral = (($back_goods['integral_money']*$back_goods['back_goods_number'])/$scale)*100;
			$integral = min($integral,$orderinfo['integral']);
			$money_paid -=  round(($integral / 100) * $scale, 2);
		}
	}

	if($orderinfo['surplus']>0){//订单是否有余额可以退
		$surplus = min($orderinfo['surplus'],$money_paid);
		$money_paid -= $surplus;
	}
	return array('money_paid'=>$money_paid,'surplus'=>$surplus,'integral'=>$integral);
}
       ...
       $res = $db->getAll($sql);
        ...
        foreach ($item_list as $key => $value) {
            ...
            if ($comment['comment_id'] > 0) {
                ...
                $comment_added=$db->getRow("SELECT * FROM ".$ecs->table('comment_added')." where status=1 And comment_id=".$comment['comment_id']);
                $comment['comment_added']=$comment_added;
            }
            //print_r($comment);die();
            //判断是否可以修改和追加 By Freely
			///print_r($comment['add_time']);print_r('=');
            $comment['is_edit']=($comment['is_edit']==0&&$comment['add_time']>=$min_editdays)?1:0;
            $comment['is_zhuijia']=$comment['add_time']>=$min_zhuijiadays?1:0;

            $item_list[$key]['comment'] = $comment;
        }
        //print_r($min_editdays);
        //print_r($item_list);die();
...
function action_comment_edit_save()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    $status = ($_CFG['comment_check'] == 1) ? 0 : 1;
    $ip_address = real_ip();
    $db->query("UPDATE " . $ecs->table('comment') . " SET is_edit=1,edit_time='".gmtime()."',content='".$content."',ip_address='".$ip_address."',status='".$status."' WHERE comment_id=".$res_comment['comment_id']);
    clear_cache_files();
    if($status == 0)
    {
        $msg = '您的信息提交成功，需要管理员审核后才能显示！';
...
function action_my_comment_send()
{
	...
	$rec_id = $_POST['rec_id'];
        ...
        //判断是否已经评价
        $res_comment = $db->getRow("select comment_id,is_edit,add_time from ".$ecs->table('comment') ." where rec_id = $rec_id and user_id = $user_id");
        if($res_comment){
...
function action_order_detail()
{
	...
	/* 订单 支付 配送 状态语言项 */
	$order['card_item_status'] = $order['pay_status'];
	$order['order_status'] = $_LANG['os'][$order['order_status']];
	$order['pay_status'] = $_LANG['ps'][$order['pay_status']];
	//if($order['is_ipc_shopping'] == 2 && $order['shipping_status'] == 1){
		//$order['shipping_status'] = '待确认';
	//}elseif($order['is_ipc_shopping'] == 2 && $order['shipping_status'] == 2){
		//$order['shipping_status'] = '已确认';
    //}else{

	//if(count($order['card_item']) == 0){
...
//	}
	...
	if(isset($order['card_item']) && !empty($order['card_item']) && $order['card_item_status']>0){
		$dirpath = $_SERVER['DOCUMENT_ROOT']."/data/images/card_item_qrcode/";
		if(!file_exists($dirpath)){
			mkdir($dirpath, 0777);
		}
		if(!file_exists($dirpath.'orderid_'.$order['order_id'].'.png')){
			require(dirname(__FILE__) . '/includes/phpqrcode.php');
			$url = "http://". $_SERVER ['HTTP_HOST'].'/mobile/supplier/order.php?act=column_sign&order_id='.$order['order_id'];
			QRcode::png($url, $dirpath.'orderid_'.$order['order_id'].'.png',"L",6,2);
		}
		$order['card_item_qrcode'] = '../data/images/card_item_qrcode/'.'orderid_'.$order['order_id'].'.png';
	}
...
function action_set_address(){
	...
	$ecs = $GLOBALS['ecs'];
        ...
        if($db->query("UPDATE " . $ecs->table('users') . " set address_id = $address_id  WHERE user_id='$user_id'")){
			if(ODOO_ERP){
				//edit yhy 同步erp中的会员
				$res = $GLOBALS['odooErpObj']->syncAddressByAddressid($address_id);
			}
...
function action_drop_consignee()
{
	...
	if(drop_consignee($consignee_id))
	{
		if(ODOO_ERP){
			//edit yhy 同步erp中的会员
			$res = $GLOBALS['odooErpObj']->syncAddressUnlinkByAddressid($consignee_id);
		}
...
function action_affirm_received()
{
	...
	if(affirm_received($order_id, $user_id))
	{
		if($GLOBALS['_CFG']['distrib_style'] == 0)
		{
			...
			distrib_separate($order_id);
			supplier_separate_by_orderid($order_id);
			
			//get_pay_fencheng($order_id,1);
			//gets_split_money_by_order_id($order_id);
			//确认收货，自动分成
//			$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
...
function action_act_account()
{
	...
	if($surplus['process_type'] == 1)
	{
        ...
        if($pay_typ==1){
            ...
            //判断当用户选择微信时判断是否有绑定账号
            $wxInfo=$GLOBALS['db']->getRow("SELECT * FROM ".$GLOBALS['ecs']->table('weixin_user')." WHERE ecuid=".$user_id);
            if(!$wxInfo){sys_msg("您的账号没有绑定微信，无法使用微信提现处理！");}
            $pay_number=$wxInfo['fake_id'];
            $pay_name='';
        }
        $surplus['pay_typ']=$pay_typ;
        $surplus['pay_number']=$pay_number;
        $surplus['pay_name']=$pay_name;
		/* 判断是否有足够的余额的进行退款的操作 */
...
function action_act_edit_surplus()
{
    ...
    //if($order['order_amount'] == 0){//如果都付款了，直接跳转到分享页面
        //ecs_header('Location: fx.php?u='.$_SESSION['user_id'].'&oid=' . $order_id . "\n");
	//}
	/* 跳转 */
