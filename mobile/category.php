...
if($user_rank>0){
    $user_rank_wh=' (find_in_set('.$user_rank.',g.level) or  g.level=0) and ';
}else{
    $user_rank_wh=' g.level=0 and ';
}
/* 排序、显示方式以及类型 */
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax') {
    ...
    $goodslist = category_get_goods($children, $other_has, $other_youhuo, $brand, $price_min, $price_max,$ext, $size, $page, $sort, $order, $limit,$user_rank_wh);
...
if (!$smarty->is_cached('category.dwt', $cache_id)) {
    ...
    if ($cat['grade'] > 1) {
        ...
        $sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max " .
                ...
                " WHERE  ".$user_rank_wh." ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  ';
        //获得当前分类下商品价格的最大值、最小值
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num " .
            ...
            "WHERE ".$user_rank_wh." g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ") AND b.is_show = 1 " .
...
//LSJ End
    ...
    $smarty->assign('best_goods', get_category_recommend_goods('best', $children, $brand, $price_min, $price_max, $ext,$user_rank_wh));
    $smarty->assign('promotion_goods', get_category_recommend_goods('promote', $children, $brand, $price_min, $price_max, $ext,$user_rank_wh));
    $smarty->assign('hot_goods', get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext,$user_rank_wh));

    $count = get_cagtegory_goods_count($children, $brand, $other_has, $price_min, $price_max, $ext,$user_rank_wh);
 ...
 * @return  array
 */
function category_get_goods($children, $other_has, $other_youhuo, $brand, $min, $max, $ext, $size, $page, $sort, $order, $limit = '',$user_rank_wh) {
    $display = $GLOBALS['display'];
    $where = $user_rank_wh. "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if(is_array($jiage)){
            if(count($jiage) == 2){
                ...
                $arr[$row['goods_id']]['final_price'] =  price_format($jiage[0]).'~'.price_format($jiage[1]);

            }else{
               
                $arr[$row['goods_id']]['final_price'] = price_format($jiage[0]);     
                
            }
        }else{

           $arr[$row['goods_id']]['final_price'] = price_format($row['shop_price']);
        }
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($children, $brand = 0, $other_has, $min = 0, $max = 0, $ext = '',$user_rank_wh) {
    $where = $user_rank_wh."g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
