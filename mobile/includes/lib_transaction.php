...
function get_user_orders_ajax($user_id, $limit,$where='')
{
    /* 取得订单列表 */
    $arr    = array();
    $sql = "SELECT o.*,c.is_ipc_shopping, ifnull(ssc.value,'网站自营') as shopname, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info') . ' as o LEFT JOIN ' .$GLOBALS['ecs']->table('supplier') .' c ON o.supplier_id=c.supplier_id '.
    ...
    {
		//卡项订单 edit yhy 2019/8/2
		$item_count = $GLOBALS['db']->getOne("SELECT COUNT(a.rec_id) FROM ".$GLOBALS['ecs']->table('order_goods')." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE b.item_id>0 AND a.order_id=".$row['order_id']);
        if ($row['order_status'] == OS_UNCONFIRMED)
        {            
        ...
        {  
            ...
            if ($row['shipping_status'] == SS_SHIPPED)
            {
				...
				//技师店铺的订单,去掉查看物流
				//if($row['is_ipc_shopping'] == 2){
                    //@$row['handler'] .= " <a style='background:#E71F19; color:#fff; border:1px solid #E71F19' href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$back_info."是否确认订单完成')) return false;\">订单确认</a><i>还剩" . $row_time . "天自动确认</i>";
				//}else{
				if($item_count == 0){
					if($order['is_ipc_shopping'] != 2){
                ...
                else
                {
				   ...
				   $status_back=$zt['status_back'];
					...
					if (!empty($status_back))
					{
						...
						//edit by yhy 订单如果是卡项订单的话，不允许申请退款
		...
		//if($row['is_ipc_shopping'] == 2 && $row['shipping_status']==1){
			//$row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']] . ',' . $GLOBALS['_LANG']['ps'][$row['pay_status']] . ',待确认';
		//}else{
		//if($item_count == 0){
			$row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']] . ',' . $GLOBALS['_LANG']['ps'][$row['pay_status']] . ',' . $GLOBALS['_LANG']['ss'][$row['shipping_status']];
		//}else{
...
function get_order_detail($order_id, $user_id = 0)
{
	...
	if($order['supplier_id']>0){
		$order['referer'] = $GLOBALS['db']->GetOne("SELECT value FROM ".$GLOBALS['ecs']->table('supplier_shop_config') ." WHERE supplier_id = '$order[supplier_id]' AND code = 'shop_name'");
	}
...
function get_order_handler($order){
	if(function_exists('is_card_item_order')){
		$item_count = is_card_item_order($order['order_id']);
	}else{
		$item_count = $GLOBALS['db']->getOne("SELECT COUNT(a.rec_id) FROM ".$GLOBALS['ecs']->table('order_goods')." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE b.item_id>0 AND a.order_id=".$order['order_id']);
	}
    ...
    if ($order['order_status'] == OS_UNCONFIRMED)
    ...
    else if ($order['order_status'] == OS_SPLITED || $order['order_status'] == OS_CONFIRMED)
        {
            ...
            if ($order['shipping_status'] == SS_SHIPPED)
            {
				...
				//if($order['is_ipc_shopping'] == 2){
					//$handler['handler'] .= " <a href=\"user.php?act=affirm_received&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$back_info."是否确认订单')) return false;\">订单确认</a>";
				//}else{
				
				if($order['is_ipc_shopping'] != 2 && $item_count == 0){
					@$okgoods_time = $GLOBALS['db']->getOne("select value from " . $GLOBALS['ecs']->table('shop_config') . " where code='okgoods_time'");
...
function get_technican_list($supplier_id){
	$sql = "SELECT id,technican_name,technican_sn FROM ".$GLOBALS['ecs']->table('technican')." WHERE supplier_id = ".$supplier_id;
