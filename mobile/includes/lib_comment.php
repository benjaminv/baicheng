...
{
	$item_id = $GLOBALS['db']->getOne("SELECT item_id FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$goods_id);
	if($item_id>0){
	...
	if($item_id>0){
		$res = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['ecs']->table('goods_tag')." WHERE goods_id IN (".implode(',',$item_goods_ids).") AND state = 1");
	}else{
	...
	if ($type != 4)
	{
		...
		if($item_id>0){
			$tag_name_c = $GLOBALS['db']->getOne("select tag_id from " . $GLOBALS['ecs']->table('goods_tag') . " where goods_id IN (".implode(',',$item_goods_ids).") and tag_name = '" . $c_tag . "'");
		}else{
		...
		if($item_id>0){
			$count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('comment')." AS c 
										 ...
										 WHERE c.id_value IN (".implode(',',$item_goods_ids).") AND c.status = 1 AND c.comment_rank > 0 $where");
		}else{
			$count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('comment')." AS c 
										 LEFT JOIN ".$GLOBALS['ecs']->table('shaidan')." AS s ON c.rec_id=s.rec_id
		...
		if($item_id>0){
			$sql = "SELECT c.*, u.headimg, s.shaidan_id, s.status AS shaidan_status,s.title,s.message FROM ".$GLOBALS['ecs']->table('comment')." AS c 
				...
				LEFT JOIN ".$GLOBALS['ecs']->table('shaidan')." AS s ON c.rec_id=s.rec_id
				WHERE c.id_value IN (".implode(',',$item_goods_ids).") AND c.status = 1 AND c.comment_rank > 0 $where ORDER BY c.add_time DESC";
		}else{
			$sql = "SELECT c.*, u.headimg, s.shaidan_id, s.status AS shaidan_status,s.title,s.message FROM ".$GLOBALS['ecs']->table('comment')." AS c 
				LEFT JOIN ".$GLOBALS['ecs']->table('users')." AS u ON c.user_id=u.user_id
				LEFT JOIN ".$GLOBALS['ecs']->table('shaidan')." AS s ON c.rec_id=s.rec_id
