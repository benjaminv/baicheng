...
function order_info($order_id, $order_sn = '')
{
	...
	$order['is_ipc_shopping'] = $GLOBALS['db']->getOne('SELECT is_ipc_shopping FROM '.$GLOBALS['ecs']->table('supplier').' WHERE supplier_id = '.$order['supplier_id']);
    ...
    if ($order)
    {
		...
		$card_item = $GLOBALS['db']->getAll("SELECT a.*,b.supplier_name,c.region_name as city_name,d.region_name as district_name,b.address,b.contacts_phone FROM ".$GLOBALS['ecs']->table('card_item_goods')." a LEFT JOIN ".$GLOBALS['ecs']->table('supplier')." b ON a.supplier_id = b.supplier_id LEFT JOIN ".$GLOBALS['ecs']->table('region')." c ON b.city=c.region_id LEFT JOIN ".$GLOBALS['ecs']->table('region')." d ON b.district=d.region_id  WHERE a.order_id=".$order_id);
		if($card_item){
			foreach($card_item as $v){
				if(!isset($order['card_item'][$v['supplier_id']]) || empty($order['card_item'][$v['supplier_id']])){
					$order['card_item'][$v['supplier_id']]['supplier_name'] = $v['supplier_name'];
					$order['card_item'][$v['supplier_id']]['address'] = $v['city_name'].$v['district_name'].$v['address'];
					$order['card_item'][$v['supplier_id']]['contacts_phone'] = $v['contacts_phone'];
				}
				$order['card_item'][$v['supplier_id']]['item_column'][] = $v;
			}
		}
...
//	}
    $result=$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'),
        $order, 'UPDATE', "order_id = '$order_id'");
    //log_supplier_commission($order_id);/*店铺佣金和地推团队处理 By Freely*/
    return $result;
}
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$supplier_id)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                ...
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price,g.supplier_id ".
    ...
    $parent = array(
        ...
        'sale_point'=>get_sale_point_by_id($goods_id,$num,$spec),
        'level_point'=>get_level_point_by_id($goods_id,$num,$spec),
        'supplier_split_money'=>get_supplier_split_money_by_id($goods_id,$num,$spec),
        'supplier_level_fuwufe'=>get_supplier_level_fuwufe_by_id($goods_id,$num,$spec),
		'supplier_id'	=> (isset($supplier_id) && !empty($supplier_id)) ? $supplier_id : ((isset($goods['supplier_id']) && !empty($goods['supplier_id'])) ? $goods['supplier_id'] : 0)
    );
    //print_r($parent);die();
...
//获取店铺分成金额
function get_supplier_split_money_by_id($goods_id,$num,$spec){
    $dianpu_money=0;
    $shop_price = get_final_price($goods_id, $num, true, $spec);
    $affiliate_supplier = unserialize($GLOBALS['_CFG']['affiliate_supplier']);
    //print_r($affiliate_supplier);die();
    if($affiliate_supplier){
        $sql="SELECT s.supplier_id,u.user_id,u.parent_id FROM ".$GLOBALS['ecs']->table('supplier')." s LEFT JOIN ".$GLOBALS['ecs']->table('users').' as u ON u.supplierId=s.supplier_id WHERE u.supplierId=s.supplier_id And u.user_id='.$_SESSION['user_id'];
        $buy_supplier=$GLOBALS['db']->getRow($sql);
        //print_r($buy_supplier);die();
        if($buy_supplier){
            //它的上级，如果有上级表示此用户是二级用户，否则就是一级用户，对应不同级的店铺分成
            $user_parent_supplier=null;
            if($buy_supplier['parent_id']){
                $sql="SELECT s.supplier_id,u.user_id,u.parent_id FROM ".$GLOBALS['ecs']->table('supplier')." s LEFT JOIN ".$GLOBALS['ecs']->table('users').' as u ON u.supplierId=s.supplier_id WHERE u.supplierId=s.supplier_id And u.user_id='.$buy_supplier['parent_id'];
            ...
            $sql = "SELECT g.*,s.supplier_name FROM ".$GLOBALS['ecs']->table('goods')." g LEFT JOIN ".$GLOBALS['ecs']->table('supplier')." as s ON s.supplier_id=g.supplier_id WHERE s.supplier_id=g.supplier_id And g.goods_id=$goods_id";
            $goods = $GLOBALS['db']->getRow($sql);
            //print_r($goods);die();
            if($goods){
                //店铺分成处理,按单个商品来处理
                ...
                $dianpu_money=($shop_price*($dianpu_fx/100));//店铺可得
            }
        }
    }
    return $dianpu_money;
...
//获取店铺服务费
function get_supplier_level_fuwufe_by_id($goods_id,$num,$spec){
    $fuwufe_money=0;
    $affiliate_supplier = unserialize($GLOBALS['_CFG']['affiliate_supplier']);
    if($affiliate_supplier){
        $sql="SELECT s.supplier_id,u.user_id,u.parent_id FROM ".$GLOBALS['ecs']->table('supplier')." s LEFT JOIN ".$GLOBALS['ecs']->table('users').' as u ON u.supplierId=s.supplier_id WHERE u.supplierId=s.supplier_id And u.user_id='.$_SESSION['user_id'];
        ...
        if($buy_supplier){
            ...
            //print_r($level_fuwufe);die();
            $sql = "SELECT g.*,s.supplier_name FROM ".$GLOBALS['ecs']->table('goods')." g LEFT JOIN ".$GLOBALS['ecs']->table('supplier')." as s ON s.supplier_id=g.supplier_id WHERE s.supplier_id=g.supplier_id And g.goods_id=$goods_id";
            $goods = $GLOBALS['db']->getRow($sql);
            if($goods){
                //当店铺不一样，才有给购买店铺加服务费
                if($buy_supplier['supplier_id']!=$goods['supplier_id']){
                    $fuwufe_money=$level_fuwufe;
                }
            }
        }
    }
...
//获取商品分成消费积分
function get_sale_point_by_id($goods_id,$num,$spec)/*微分销方法调整，全部替换即可*/
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('ecsmart_distrib_goods') . " WHERE goods_id = '$goods_id' AND (distrib_time = 0 OR (start_time <= " . gmtime() . " AND end_time >= " .gmtime()."))";
    $rows = $GLOBALS['db']->getRow($sql);
    if($rows)
    {
        if($rows['distrib_sale_type'] == 1 && $rows['distrib_sale_money'] > 0)
        {
            $sale_point = $rows['distrib_sale_money'];
        }

        if($rows['distrib_sale_type'] == 2 && $rows['distrib_sale_money'] > 0)
        {
            $shop_price = get_final_price($goods_id, $num, true, $spec);
            $sale_point = $shop_price*$rows['distrib_sale_money']/100;
        }
        return $sale_point;
    }
    else
...
//获取商品分成等级积分
function get_level_point_by_id($goods_id,$num,$spec)/*微分销方法调整，全部替换即可*/
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('ecsmart_distrib_goods') . " WHERE goods_id = '$goods_id' AND (distrib_time = 0 OR (start_time <= " . gmtime() . " AND end_time >= " .gmtime()."))";
    $rows = $GLOBALS['db']->getRow($sql);
    if($rows)
    {
        if($rows['distrib_level_type'] == 1 && $rows['distrib_level_money'] > 0)
        {
            $level_point = $rows['distrib_level_money'];
        }

        if($rows['distrib_level_type'] == 2 && $rows['distrib_level_money'] > 0)
        {
            $shop_price = get_final_price($goods_id, $num, true, $spec);
            $level_point = $shop_price*$rows['distrib_level_money']/100;
        }
        return $level_point;
    }
    else
...
function del_cart_goods()
{
    ...
    $user_rank = $GLOBALS['db']->getOne("SELECT user_rank FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id='".$_SESSION['user_id']."'");

    /* 循环、统计 */
    $sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.*,g.level, g.cat_id, g.brand_id, c.extension_code, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid  " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        ksort($goods_list);
        //判断是否有权限购买此商品，如果没有删除购物车
        foreach($goods_list as $k=>$v){
            if(!empty($v['goods_list'])){
                foreach($v['goods_list'] as $k1=>$v1){

                    if(!in_array($user_rank,explode(',',$v1['level']))&&$v1['level']!=0){
                        $sql = "DELETE FROM " .$GLOBALS['ecs']->table('cart') . " WHERE rec_id = '$v1[rec_id]'";
                        $res=$GLOBALS['db']->query($sql);
...
{
    del_cart_goods();
	del_cart_by_user_supplier(intval($_SESSION['user_id']));
    /* 初始化 */
    $goods_list = array();
    $total = array(
        'goods_price'  => 0, // 本店售价合计（有格式）
 ...
 * @return  bool
 *
 *edit by yhy 2019/6/10查询购物车或订单中是否有
 */

function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS,$supplier_id = -1,$sel_cartgoods="")
{
	
    if ($order_id <= 0){
		$sql_where = $_SESSION['user_id']>0 ? "a.user_id='". $_SESSION['user_id'] ."' " : "a.session_id = '" . SESS_ID . "' AND a.user_id=0 ";
		
        $sql = "SELECT COUNT(a.goods_id) FROM " . $GLOBALS['ecs']->table('cart') .
                " a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE " . $sql_where . " AND a.is_real = 1 AND b.is_service=0 " .
                "AND a.rec_type = '$flow_type'";
		if($sel_cartgoods){
			$sql .= " AND a.rec_id in (". $sel_cartgoods .") ";
		}elseif ($_SESSION['sel_cartgoods']){
			$sql .= " AND a.rec_id in (". $_SESSION['sel_cartgoods'] .") ";
		}
    }else{
        $sql = "SELECT COUNT(a.goods_id) FROM " . $GLOBALS['ecs']->table('order_goods') ." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id".
                " WHERE a.order_id = '$order_id' AND a.is_real = 1 AND b.is_service=0 ";
    }
	if($supplier_id >= 0){
...
**是否存在平台的服务
**
**edit by yhy 2019/6/12
*/
function exist_service_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS,$supplier_id = -1){
	if ($order_id <= 0)
    {
		$sql_where = $_SESSION['user_id']>0 ? "a.user_id='". $_SESSION['user_id'] ."' " : "a.session_id = '" . SESS_ID . "' AND a.user_id=0 ";
		
        $sql = "SELECT COUNT(a.goods_id) FROM " . $GLOBALS['ecs']->table('cart') .
                " a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE " . $sql_where . " AND a.is_real = 1 AND b.is_service=1 " .
                "AND a.rec_type = '$flow_type'";
		if ($_SESSION['sel_cartgoods'])
		{
			$sql .= " AND a.rec_id in (". $_SESSION['sel_cartgoods'] .") ";
    ...
    {
        $sql = "SELECT COUNT(a.goods_id) FROM " . $GLOBALS['ecs']->table('order_goods') ." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id".
                " WHERE a.order_id = '$order_id' AND a.is_real = 1 AND b.is_service=1 ";
    }
	if($supplier_id >= 0){
			$sql .= " AND b.supplier_id = ".$supplier_id;
	}
...
/*
**是否存在技师店铺的服务 存在的话返回技师的行程安排 
**
**edit by yhy 2019/7/15
*/
function exist_technican_goods($flow_type = CART_GENERAL_GOODS,$supplier_id = 0){
	
	$sql_where = $_SESSION['user_id']>0 ? "a.user_id='". $_SESSION['user_id'] ."' " : "a.session_id = '" . SESS_ID . "' AND a.user_id=0 ";
	
	$sql = "SELECT DISTINCT(a.supplier_id) FROM " . $GLOBALS['ecs']->table('cart') ." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id LEFT JOIN ".$GLOBALS['ecs']->table('supplier')." c ON a.supplier_id=c.supplier_id WHERE " . $sql_where . " AND b.supplier_id>0 AND a.is_real = 1 AND b.is_service=1 AND a.rec_type = '$flow_type' AND c.is_ipc_shopping = 2";
	if($supplier_id > 0){
		$sql .= " AND b.supplier_id = ".$supplier_id;
	}
	if ($_SESSION['sel_cartgoods'])
	...
	$supplier = $GLOBALS['db']->getOne($sql);
	if(!empty($supplier)){
		if($supplier_id>0){
			$result = get_schedul_data($supplier);
			$schedul_list = array_merge_recursive($result['technican_schedul'],$result['order_schedul']);
			return array('code'=>1,'schedul_list'=>$schedul_list);
		}else{
			return array('code'=>1);
		}
	}else{
...
function get_schedul_data($supplier_id){
	$ecs = $GLOBALS['ecs'];
	$db = $GLOBALS['db'];
	$list = $db->getAll('SELECT datetime,schedul,group_concat(timepick) as time_pick FROM '.$ecs->table('technican_schedul').' WHERE supplier_id='.$supplier_id.'  AND datetime>="'.date('Y-m-d').'" GROUP BY datetime,schedul');
	...
	//返回所有订单行程的列表
	$order = $db->getAll('SELECT datetime,schedul,timepick,order_id FROM '.$ecs->table('technican_schedul').' WHERE supplier_id='.$supplier_id.' AND schedul=1 AND schedul_status=1 AND datetime>="'.date('Y-m-d').'"');
	foreach($order as $k=>$v){
		$timepick = explode(',',$v['timepick']);
		foreach($timepick as $tv){
			$technican_order[$v['datetime']][$tv]['order_id'] = $v['order_id'];
			$technican_order[$v['datetime']][$tv]['order_sn'] = $db->getOne('SELECT order_sn FROM '.$ecs->table('order_info').' WHERE order_id='.$v['order_id']);
		}
	}
...
//获取某技师未来30天的行程
function get_future_schedul($supplier_id){
	$schedul_data = array(
		'08:00'=>'hour1',
		'08:30'=>'hour1',
		'09:00'=>'hour1',
	...
	$local_date = local_getdate();
	for($i=0;$i<30;$i++){
		if($i==0){
			//已经过去的时间设置为不可预约
			foreach($schedul_data as $k=>$v){
				$time_arr = explode(':',$k);
				if((intval($local_date['hours']) > intval($time_arr[0])) || (intval($local_date['hours']) == intval($time_arr[0]) && intval($local_date['minutes']) >= intval($time_arr[1]))){
					$schedul_init[date('Y-m-d')][$k] = 'hour0';
				}else{
					$schedul_init[date('Y-m-d')][$k] = $v;
				}
			}
		}else{
			$schedul_init[date('Y-m-d',strtotime('+'.$i.'days'))] = $schedul_data;
		}
	}
	$res = get_schedul_data($supplier_id);
	$schedul_list = array_merge_recursive($res['technican_schedul'],$res['order_schedul']);
	foreach($schedul_init as $k=>$v){
		if(isset($schedul_list[$k]) && !empty($schedul_list[$k])){
			$init_keys = array_keys($v);
			foreach($schedul_list[$k] as $val){
				if(in_array($val,$init_keys)){
					$schedul_init[$k][$val] = 'hour0';
				}
			}
		}
		$schedul_init[substr($k,5)] = $schedul_init[$k];
		unset($schedul_init[$k]);
	}
	return $schedul_init;
}
...
function cart_weight_price2($type = CART_GENERAL_GOODS, $supplier_id)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
	$sql_where = $_SESSION['user_id']>0 ? " AND a.user_id='". $_SESSION['user_id'] ."' " : " AND a.session_id = '" . SESS_ID . "' AND a.user_id=0 ";
    $sql = 'SELECT a.goods_id, a.goods_number, a.goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE a.extension_code = 'package_buy' AND b.is_service=0 ".$sql_where;
    $row = $GLOBALS['db']->getAll($sql);
    if ($row)
    {
        $packages_row['free_shipping'] = 0;
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                "AND rec_type = '$type' AND g.is_shipping = 0 AND c.extension_code != 'package_buy' AND g.is_service=0";
	if($_SESSION['user_id'] > 0){
		$sql .= " AND c.user_id=".$_SESSION['user_id'];
	}else{
		$sql .= " AND c.user_id=0 AND c.session_id='".SESS_ID."'".$_SESSION['user_id'];
	}
    $row = $GLOBALS['db']->getRow($sql);
...
//根据会员id获取用户绑定的店铺，如果为空返回默认店铺 cookie_flag是否忽略cookie，默认不忽略
function get_user_default_supplier($user_id=0,$cookie_flag = false){
	$_CFG = $GLOBALS['_CFG'];
	$db = $GLOBALS['db'];
	$ecs = $GLOBALS['ecs'];
	$default_supplier_id = 0;
	$cookie_supplier = intval($_COOKIE['u_supplier_id']);
	$cookie_default_supplier = $db->getOne( 'SELECT supplier_id FROM ' . $ecs->table("supplier") . '  WHERE supplier_id = '.$cookie_supplier);
	if(!$cookie_flag && $cookie_supplier>0 && isset($cookie_default_supplier) && !empty($cookie_default_supplier)){
		$default_supplier_id = $cookie_supplier;
	}else{
		$user_supplier_id = $db->getOne( 'SELECT a.supplierId FROM ' . $ecs->table("users") . ' a LEFT JOIN ' . $ecs->table("supplier") . ' b ON a.supplierId=b.supplier_id  WHERE a.user_id = '.$user_id.' AND IFNULL(b.supplier_id,0) > 0', true);
		if(!empty($user_supplier_id) && $user_supplier_id>0){
			$default_supplier_id = $user_supplier_id;
		}else{
			//进入默认店铺，改为进入用户关联的店铺
			$affiliate_supplier = unserialize($_CFG['affiliate_supplier']);
			$cfg_supplier_id = $db->getOne('SELECT supplier_id FROM ' . $ecs->table("supplier") . '  WHERE supplier_id = '.intval($affiliate_supplier['config']['default_reg_supplier_id']), true);
			if(!empty($cfg_supplier_id) && $cfg_supplier_id>0){
				$default_supplier_id = $cfg_supplier_id;
			}
		}
	}
...
function del_cart_by_user_supplier($user_id=0){
	$db = $GLOBALS['db'];
	$default_supplier_id = get_user_default_supplier(intval($user_id));
	$cart_sql = "DELETE a FROM ". $GLOBALS['ecs']->table('cart') ." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE a.user_id=".$user_id." AND b.supplier_id<>".$default_supplier_id." AND b.supplier_id <>0";
	$db->query($cart_sql);
}


//如果订单为卡项订单，新增卡项订单关联表数据 2019/8/2 edit yhy 卡项只能单独下单
function card_item_order_goods($order_id){
	$item_id = $GLOBALS['db']->getOne("SELECT b.item_id FROM ".$GLOBALS['ecs']->table('order_goods')." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE b.item_id>0 AND a.order_id=".$order_id);
	if($item_id){
		$item_columns = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['ecs']->table('card_item_column')." WHERE item_id=".$item_id);
		$sql = "INSERT INTO ".$GLOBALS['ecs']->table('card_item_goods')." (item_id,supplier_id,column_id,order_id,column_name,column_number) VALUES ";
		foreach($item_columns as $v){
			$sql .= "(".$item_id.",".$v['supplier_id'].",".$v['id'].",".$order_id.",'".$v['project_name']."','".$v['project_number']."'),";
		}
		$sql = substr($sql,0,-1);
		$GLOBALS['db']->query($sql);
	}
...
function is_card_item_order($order_id){
		$item_count = $GLOBALS['db']->getOne("SELECT COUNT(a.rec_id) FROM ".$GLOBALS['ecs']->table('order_goods')." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE b.item_id>0 AND a.order_id=".$order_id);
		return $item_count;
}
