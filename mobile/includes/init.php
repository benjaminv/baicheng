...
if(($ua == '' || preg_match($uachar, $ua))&& !strpos(strtolower($_SERVER['REQUEST_URI']),'wap')) {
    ...
    if($is_weixin_browser && (!isset($_SESSION['user_id']) ||empty($_SESSION['user_id']))){
        if(strpos($_SERVER['PHP_SELF'],'weixin_login.php') === false && strpos($_SERVER['PHP_SELF'],'sms.php') === false){
            $url = $GLOBALS['ecs']->url()."weixin_login.php?erweima_type=1&user_id=".intval($_GET['parent_id']);
            if(isset($_GET['parent_id']) && $_GET['parent_id']>0){
                $url .= "&user_id=".intval($_GET['parent_id']);
            }else{
                $url .= "&user_id=0";
            }
