...
//                $cat_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
                $cat_arr[$row['cat_id']]['url']  = 'category.php?category='.$row['cat_id'].'&display=grid&brand=0&price_min=0&price_max=0&filter_attr=0&page=1&sort=goods_id&order=ASC#goods_list';
...
//               $three_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
            $three_arr[$row['cat_id']]['url']  = 'category.php?category='.$row['cat_id'].'&display=grid&brand=0&price_min=0&price_max=0&filter_attr=0&page=1&sort=goods_id&order=ASC#goods_list';
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ...
               ' WHERE '.$user_rank_wh.'g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
        ...
        $sql .= ' WHERE '.$user_rank_wh.'g.goods_id ' . db_create_in($type_merge);
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE '.$user_rank_wh.'g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='',$user_rank_wh)
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE '.$user_rank_wh.'g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . $brand_where . $price_where . $ext;
    $num = 0;
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        $jiage = get_price_shuxing($row['shop_price'],$goods_id);
       
        if(is_array($jiage)){
            if(count($jiage) == 2){

                $row['shop_price_formated'] =  price_format($jiage[0]).'~'.price_format($jiage[1]);

            }else{
               
                $row['shop_price_formated'] = price_format($jiage[0]);     
...
function get_goods_gallery($goods_id)
{
    ...
    foreach($row as $key => $gallery_img)
    {
        ...
        $row[$key]['img_original'] = get_pc_url().'/'.get_image_path($goods_id, $gallery_img['img_original'], false, 'gallery');
		//yyy修改end
    }
    return $row;
}
 ...
 *
 */
function get_price_shuxing($price,$goods_id){

    $sql = 'select attr_price from ecs_goods_attr where goods_id="'.$goods_id.'"';
    $prices = $GLOBALS['db']->getAll($sql);
    if($prices){

        $arr = []; 
        foreach ($prices as $val) {
            array_push($arr, $price+$val['attr_price']); 
        }
        $max = max($arr);
        $min = min($arr);
        if($max == $min){
