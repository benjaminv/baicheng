    ...
    if ($log_id > 0)
    {
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
	      ...
	      //palenggege修改结束
					...
					//订单中没有实体商品的话，更改订单状态为已收货已完成,并发放提成
					//if(!exist_real_goods($order_id)){
							//$sql = 'update ecs_order_info set order_status=5,shipping_status=2,pay_status=2,is_separate=1,shipping_time_end="'.gmtime().'" where order_id='.$order_id;
							//$GLOBALS['db']->query($sql);
