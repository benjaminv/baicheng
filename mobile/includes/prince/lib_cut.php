...
function get_cut_info($id)
{
	...
	global $db;
	global $_CFG;
	$sql = 'SELECT act_id, act_name AS cut_name, goods_id, product_id, goods_name, start_time, end_time, act_desc, ext_info, s.supplier_id,s.supplier_name' . ' FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS g LEFT JOIN ' . $GLOBALS['ecs']->table('supplier') . ' AS s ON s.supplier_id=g.supplier_id  WHERE act_id=\'' . $id . '\' AND act_type = ' . GAT_CUT;
	$cut = $db->GetRow($sql);
	$cut['start_time'] = local_date('Y-m-d H:i', $cut['start_time']);
	$cut['end_time'] = local_date('Y-m-d H:i', $cut['end_time']);
