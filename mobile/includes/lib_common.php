...
/**
 * 地推团队佣金记录变动
 * @param   float   $commission_money     佣金变动
 * @param   string  $change_desc    变动说明
 * @param   int     $change_type    变动类型：参见常量文件
 * @return  void
 */
function push_team_log_account_change($commission_money = 0, $change_desc = '', $change_type = PT_OTHER)
{
    /* 插入变动记录 */
    $account_log = array(
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('push_team_account_log'), $account_log, 'INSERT');
    $sql = "UPDATE " . $GLOBALS['ecs']->table('ecsmart_shop_config',1) .
        " SET value = value + ('$commission_money')" .
        " WHERE code = 'push_team_commission_money' LIMIT 1";
    $GLOBALS['db']->query($sql);
}
 ...
 * @param   string  $change_desc    变动说明
 * @param   int     $change_type    变动类型：参见常量文件
 * @param   int     $updatesql      更新其它字段
 * @return  void
 */
function supplier_log_account_change($supplier_id, $commission_money = 0, $change_desc = '', $change_type = SUPPLIER_OTHER,$updatesql='')
{
    /* 插入变动记录 */
    ...
    //print_r($supplier_account_log);die();
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('supplier_account_log'), $supplier_account_log, 'INSERT');
    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table('supplier') .
        " SET commission_money = commission_money + ('$commission_money')" .$updatesql.
...
function add_default_consignee($user_id,$supplier_id){
	$supplier_info = $GLOBALS['db']->getRow("SELECT country,province,city,district,address FROM ".$GLOBALS['ecs']->table('supplier')." WHERE supplier_id=".$supplier_id);
	$user_info = $GLOBALS['db']->getRow("SELECT user_name,mobile_phone FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id=".$user_id);
	if(!empty($supplier_info) && !empty($user_info)){
		$save_data = array(
			'address_name'	=>'归属店铺地址',
			'consignee'		=>$user_info['user_name'],
			'country'		=>$supplier_info['country'],
			'province'		=>$supplier_info['province'],
			'city'			=>$supplier_info['city'],
			'district'		=>$supplier_info['district'],
			'address'		=>$supplier_info['address'],
			'mobile'		=>$supplier_info['mobile_phone'],
			'user_id'		=>$user_id
		);
		$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_address'), $save_data, 'INSERT', '', 'SILENT');
	}
}
?>