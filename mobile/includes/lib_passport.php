	...
	else
	{
		...
		if(isset($affiliate['on']) && $affiliate['on'] == 1)
		{
			...
			if($up_uid)
			{
                ...
                //设置所属店铺和上级，如果上级用户是店铺用户，则没有上级
                list($u_supplier_id,$up_uid)=get_affiliate_supplier_id($up_uid);
			}
            else
            {
                $up_uid=0;
            ...
            //print_r($u_supplier_id);die();
            // 设置推荐人
            $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $up_uid . ',supplierId=' . $u_supplier_id . ' WHERE user_id = ' . $_SESSION['user_id'];
            //print_r($sql);die();
