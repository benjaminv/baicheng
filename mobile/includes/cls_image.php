...
class cls_image
{
     ...
     * @return bool|mixed
     */
    public function upload_image_root($upload, $dir = '', $img_name = '')
    {
        /* 没有指定目录默认为根目录images */
        if (empty($dir))
        {
            /* 创建当月目录 */
            $dir = date('Ym');
            $dir = ROOT_PATH_WAP . $this->images_dir . '/' . $dir . '/';
        }
        else
        {
            /* 创建目录 */
            $dir = ROOT_PATH_WAP . $this->data_dir . '/' . $dir . '/';
            if ($img_name)
            {
                $img_name = $dir . $img_name; // 将图片定位到正确地址
            }
        ...
        if ($this->move_file($upload, $img_name))
        {
            return str_replace(ROOT_PATH_WAP, '', $img_name);
        }
        else
        {
            $this->error_msg = sprintf($GLOBALS['_LANG']['upload_failure'], $upload['name']);
     ...
     * @return  mix         如果成功返回缩略图的路径，失败则返回false
     */
    function make_thumb_mobile($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
        $gd = $this->gd_version(); //获取 GD 版本。0 表示没有 GD 库，1 表示 GD 1.x，2 表示 GD 2.x
        if ($gd == 0)
        {
