...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points, u.reg_time,u.parent_id,u.supplierId,(select supplier_id from '.$GLOBALS['ecs']->table('supplier').' where status=1 And user_id=u.user_id) as u_supplier_id'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
//        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        if (in_array($filename, array('category', 'article_cat', 'article', 'brand')))
        {
            /* 商品分类或商品 */
    ...
    if (!empty($str))
    {
        if('extpintuan' == $filename || 'goods' == $filename){
            $page_title = $str;
        }else {
...
{
	$item_id = $GLOBALS['db']->getOne("SELECT item_id FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$id);
	if($item_id>0){
		$item_goods_ids = $GLOBALS['db']->getAll("SELECT goods_id FROM ".$GLOBALS['ecs']->table('goods')." WHERE item_id=".$item_id);
		$item_goods_ids = array_column($item_goods_ids,'goods_id');
	}
	...
	if($item_id>0){
		$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value  IN (".implode(',',$item_goods_ids).") AND comment_type = '$type' AND status = 1 AND parent_id = 0 $sql_comment ");
	}else{
		$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
...
//    $smarty->assign('copyright',     sprintf($GLOBALS['_LANG']['copyright'], date('Y'), $GLOBALS['_CFG']['shop_name']));
    $smarty->assign('copyright',     sprintf($GLOBALS['_LANG']['copyright'], date('Y'), $copyright));
...
//根据推荐ID获取所属店铺，如果推荐ID是普通用户则再获取它的上一级关联的店铺，如果它上一级没有店铺关联那使用默认店铺
function get_affiliate_supplier_id($uid=0){
    $tmp_parent_id=$uid;
    $tmp_supplier_id=0;
	$is_old = 0;
    $uinfo=get_user_info($uid);//上级用户
    if($uinfo){
        //判断如果上级是店铺用户 那直接使用此上级的店铺
        if($uinfo['u_supplier_id']&&$uinfo['u_supplier_id']!=0){
            $tmp_supplier_id=$uinfo['u_supplier_id'];
			$is_old = 1;
        }else{
			/*edit by yhy 2019/7/10 店铺的会员拉来的人还是属于这个店铺*/
			$tmp_supplier_id=intval($uinfo['supplierId']);
            //判断当上级用户的上级不是平台，所属店铺为默认店铺
            //if(!empty($uinfo['parent_id'])&&$uinfo['parent_id']!=0){
                //$tmp_supplier_id=0;
            //}else{
                ////上级为平台时所属店铺就是上级的所属店铺
                //$tmp_supplier_id=$uinfo['supplierId'];
            //}
        }

		
    }
    if($tmp_supplier_id==0){
        $config = unserialize($GLOBALS['_CFG']['affiliate_supplier']);
        empty($config) && $config = array();
        $tmp_supplier_id=$config['config']['default_reg_supplier_id'];
    }
    return [$tmp_supplier_id,$tmp_parent_id,$is_old];
}
/**
 * 获取推荐uid
 *
...
//更新用户店铺关联表 edit yhy 2019/6/24
function update_user_supplier_relate($user_id=0,$supplier_id=0){
	if(!$user_id || !$supplier_id){
		return false;
	}
	$sql = 'SELECT COUNT(id) FROM ' . $GLOBALS['ecs']->table('user_supplier_relate') ." WHERE user_id=".$user_id." AND supplier_id=".$supplier_id;
	$count = $GLOBALS['db']->getOne($sql);
	if($count > 0){
		return;
	}else{
		$data=array('user_id'=>$user_id,'supplier_id'=>$supplier_id,'addtime'=>gmtime());
		$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_supplier_relate'), $data, 'INSERT', '', 'SILENT');
	}
}

function get_user_supplier_relate($user_id){
	if(!$user_id){
		return false;
	}
	$sql = 'SELECT supplier_id FROM ' . $GLOBALS['ecs']->table('user_supplier_relate') ." WHERE user_id=".$user_id;
