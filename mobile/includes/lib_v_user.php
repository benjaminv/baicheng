...
function get_split_money($user_id,$order_id)
{
	 $sql = "SELECT sum(money) FROM " .
			...
			" WHERE user_id = '$user_id' AND order_id = '$order_id' and separate_type";
...
function get_distrib_user_info($user_id,$level,$type=0)
{
	...
	if($count)
	{
		 $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,remarks ".
                    " FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id IN($up_uid) $wh";
...
function get_affiliate_info_by_user_id($user_id,$page,$size)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		{
			$arr[$row['order_id']]['set_money'] = !empty(get_total_money_by_orders($_SESSION['user_id'],$row['order_id'],$row['is_separate']))?get_total_money_by_orders($_SESSION['user_id'],$row['order_id'],$row['is_separate']):0;
		}
		else
		{
...
function get_count_distrib_order_by_user_id($user_id,$is_separate)
{
	...
	if(!empty($oids))
	{
		$sql = "SELECT COUNT(*) FROM
				(SELECT SUM(split_money) as total_money FROM " .
				$GLOBALS['ecs']->table('order_info') . " as o ," .
				$GLOBALS['ecs']->table('order_goods') . " as og where o.order_id = og.order_id AND o.order_id in($oids) AND o.pay_status = 2 AND is_separate = '$is_separate' group by o.order_id ) as oog $where";
		return $GLOBALS['db']->getOne($sql);
	}
	return 0;
}
	...
	if(!empty($uids))
	{
		...
		if(!empty($oids))
		{
			$sql = "SELECT  order_id,goods_id,goods_name,user_id," .
				...
				" AND o.pay_status = 2 AND is_separate = '$is_separate' " .
			...
			while ($row = $GLOBALS['db']->fetchRow($res))
			{
				...
				$arr[$row['order_id']]['goods_count'] = get_goods_count($row['order_id']);
				$arr[$row['order_id']]['total_split_money'] = $row['total_split_money'];
				$arr[$row['order_id']]['total_money'] = !empty(get_total_money_by_orders($user_id,$row['order_id'],$is_separate))?get_total_money_by_orders($user_id,$row['order_id'],$is_separate):0;
...
function get_total_money_by_orders($user_id,$order_id,$is_separate)
{
	 $total_money = 0;
	if($is_separate==2){
		$wh=" and separate_type in (2,3)";
	}else{
		$wh=" and separate_type= '$is_separate'";
	}
	$sql = "SELECT sum(money) FROM " .
		$GLOBALS['ecs']->table('affiliate_log') .
		" WHERE order_id = '$order_id' AND user_id = '$user_id' $wh ";
...
//		  $sql = "SELECT sum(money) FROM " .
//		  		 $GLOBALS['ecs']->table('affiliate_log') .
//		  		 " WHERE order_id = '$order_id' AND user_id = '$user_id' and separate_type= '$is_separate'";
//
//		  $total_money = $GLOBALS['db']->getOne($sql);
...
//获取用户账单总金额
function my_get_total_money($user_id)
{
	$total_money=0;
		//已分成总额
		$sql = "SELECT sum(money) FROM " . $GLOBALS['ecs']->table('affiliate_log') . " WHERE  user_id = '$user_id' and separate_type =1 and money !=0";
	$total_money += $GLOBALS['db']->getOne($sql);
	return $total_money;
}
//获取用户分成、未分成、撤销分成总金额
...
{
	$wh=' separate_type=1 and ';
	if($is_separate == 1)
	{
		//已分成总额
		$sql = "SELECT SUM(money) FROM " . $GLOBALS['ecs']->table('affiliate_log') . " WHERE ".$wh." user_id = '$user_id'";
		$total_money = $GLOBALS['db']->getOne($sql);
	}
	else if($is_separate == 2)
	{
		//撤销分成总金额
		$sql = "SELECT SUM(money) FROM " . $GLOBALS['ecs']->table('affiliate_log') . " WHERE  separate_type in (2,3) and  user_id = '$user_id' AND money < 0";
		$money = $GLOBALS['db']->getOne($sql);
		$total_money = abs($money);
	}
	else if($is_separate == 3)
	{
		//提现总金额
		$sql = "SELECT SUM(money) FROM " . $GLOBALS['ecs']->table('affiliate_log') . " WHERE   user_id = '$user_id' and  separate_type in (1,4)";
		$money = $GLOBALS['db']->getOne($sql);
		$total_money = abs($money);
	}
	else
...
//获取用户日志积分
function get_total_point_by_user_id($user_id,$is_separate,$point)
{
	$wh=' '.$point.'!=0 and  ';
	if($is_separate == 1)
	{
		//已分成总额
		$sql = "SELECT SUM($point) FROM " . $GLOBALS['ecs']->table('account_log') . " WHERE ".$wh." user_id = '$user_id'";
		$total_money = $GLOBALS['db']->getOne($sql);
	}
...
{
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('account_log') . " WHERE user_id = '$user_id'  order by change_time desc";
	$list = $GLOBALS['db']->getAll($sql);
	$arr = array();
	foreach($list as $key => $val)
	{
		...
		$arr[$key]['type'] = $type;
		$arr[$key]['money'] = $val['user_money'];
		$arr[$key]['level_point'] = $val['rank_points'];
		$arr[$key]['sale_point'] = $val['pay_points'];
		$arr[$key]['time'] = local_date("Y-m-d",$val['change_time']);
