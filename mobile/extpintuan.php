...
{   
	...
	if($extpintuan['supplier_id']>0 && $_SESSION['user_id']>0){
		setcookie('u_supplier_id', intval($extpintuan['supplier_id']), gmtime() + 3600 * 24); // 过期时间为 1 天
	}
...
elseif ($_REQUEST['act'] == 'pt_view')
{
    ...
    if (!$smarty->is_cached('extpintuan_view.dwt', $cache_id))
    {
       ...
       //order_status:0 未支付 1 已支付 2 已取消 5 商家已接单
       $sql = "SELECT pto.*,o.order_status,o.shipping_status,o.pay_status " .
            ...
            "WHERE pto.pt_id=".$extpintuan_id." and pto.follow_user=".$user_id." AND o.order_status IN(0,1,5)";
	   ...
	   $smarty->assign('my_extpintuan', $my_extpintuan);    
		...
		//echo "<pre>";print_r($pt_order_list);exit;
...
{    	
    ...
    $cart = array(
		...
		'cost_price'      =>$fencheng,
        'supplier_split_money'=>$supplier_split_money,
        'supplier_level_fuwufe'=>$supplier_level_fuwufe,
        );
