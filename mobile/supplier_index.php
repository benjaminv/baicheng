...
if (!$smarty->is_cached('mall.dwt', $cache_id))
{
	...
	$smarty->assign('stock_goods',     get_stock_goods());       //平台商品
	...
	if($suppinfo['is_ipc_shopping'] == 0 || $suppinfo['is_ipc_shopping'] == 1){
		//商圈数量
		$circle_id = $GLOBALS['db']->getOne("SELECT circle_id FROM ".$GLOBALS['ecs']->table('supplier_circle_list')." WHERE supplier_id = ".intval($_GET['suppId']));
		$smarty->assign('circle_id', $circle_id);
		
		if($circle_id>0){
			$circlenum = $GLOBALS['db']->getOne("SELECT COUNT(id) FROM ".$GLOBALS['ecs']->table('supplier_circle_list')." WHERE circle_id = ".intval($circle_id));
			$GLOBALS['smarty']->assign('circlenum', $circlenum);
		}
	}
  ...
  // 获取轮播图
    ...
    foreach ($playerdb as $key => $val) {
        // 判断图片的有限时间是否过期
        if(( $val['end'] == 0) || $val['end'] < local_date('Y-m-d H:i', gmtime()) || $val['begin'] > local_date('Y-m-d H:i', gmtime())){
...
//获取店铺进货的平台商品列表
function get_stock_goods(){
	$sql = "SELECT b.* FROM ". $GLOBALS['ecs']->table('supplier_goods') ." a LEFT JOIN ".$GLOBALS['ecs']->table('goods')." b ON a.goods_id=b.goods_id WHERE a.supplier_id=".$_GET['suppId']." AND b.is_on_sale = 1 AND b.is_alone_sale = 1 AND b.is_delete = 0 AND b.is_virtual = 0 
	ORDER BY b.sort_order, b.last_update DESC LIMIT 10"; 
...
function get_flash_xml()
{
    ...
    if (file_exists($root_path_wap . DATA_DIR . '/'.$flash_file))
    {
        ...
        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"\sbegin="([^"]*)"\s end="([^"]+)"/', file_get_contents($root_path_wap . DATA_DIR . '/'.$flash_file), $t, PREG_SET_ORDER))
        {
        ...
        if (!empty($t))
        {
            foreach ($t as $key => $val)
            {
                ...
                $val[5] = isset($val[5]) ? $val[5] : 0; // 是否时间截止
                $val[6] = isset($val[6]) ? $val[6] : 0; // 是否时间截止
                $flashdb[] = array(
                        'src'=>$val[1],
                        'url'=>$val[2],
                        'text'=>$val[3],
