...
//		$cut['gmt_end_time']= local_strtotime($cut['end_time'])+3600*8;
        ...
        $smarty->assign('weixin_qrcode',  isset($ret['guide_qrcode']) ? $ret['guide_qrcode'] : '');
...
{    
    ...
    if($cut['supplier_id']>0 && $_SESSION['user_id']>0){
        setcookie('u_supplier_id', intval($cut['supplier_id']), gmtime() + 3600 * 24); // 过期时间为 1 天
    }
    ...
    if (!$smarty->is_cached('cut.dwt', $cache_id))
    {
		...
		$cut['gmt_end_time']=$cut['end_cut_time']>gmtime()?$cut['end_cut_time']:$cut['end_buy_time'];
        ...
        $right_action='立即参与';
        $right_url="cut.php?act=join&id=".$cut['act_id'];

        $smarty->assign('right_action',  $right_action);
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    $cart = array(
        ...
        'supplier_split_money'=>0,
        'supplier_level_fuwufe'=>0,
    );
