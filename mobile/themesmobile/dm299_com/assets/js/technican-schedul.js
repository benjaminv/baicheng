...
function setTimeHtml(date_, type) {
	...
	if (nowDate[0] == date_) {
		...
		for (var i = 0; i < halfHourArray.length; i++) {
			var schedulH = parseInt(halfHourArray[i].split(':')[0]).toString();
			if(nowH>parseInt(schedulH) || (nowH == parseInt(schedulH) && parseInt(nowTime[1]) >= parseInt(halfHourArray[i].split(':')[1]))){
				returnHtml += "<div class='time-table disabled' attr-timepick='"+halfHourArray[i]+"'>" +
							"<div>" + halfHourArray[i] + "</div>" +
							"</div>";
			}else{
				var className = "";
				var orderSpan = "";
				//如果该点已经安排行程,则增加disabled标签,
				if(typeof(technican_schedul[date_]) != 'undefined'  ){
					var index = $.inArray(halfHourArray[i],technican_schedul[date_]);
					if(index >= 0){
						className += "choosed choose2";
					}
				}
				if(typeof(order_schedul[date_]) != 'undefined' ){
					var index = $.inArray(halfHourArray[i],order_schedul[date_]);
					if(index >= 0){
						className += "choosed disabled order-disabled";
						orderSpan = "<span class='font11'>订单</span>";
					}
				}
				returnHtml += "<div class='time-table "+className+" ' attr-timepick='"+halfHourArray[i]+"'>" +
							"<div>" + halfHourArray[i] +orderSpan+ "</div>" +
							"</div>";
			}
		}
	...
	if(typeof(technican_order[date_]) != 'undefined' ){
		$.each(technican_order[date_],function(i,j){
			orderHtml += '<div class="weui-cell order-toogle"><div class="weui-cell__bd"><p>订单号:<span id="order_sn">'+j.order_sn+'</span></p></div>';
			if(j.status == 0){
				orderHtml += '<a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_default" style="color:#06AE56;">待服务</a>';
			}
			if(j.status == 1){
				orderHtml += '<a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_default"  style="color:#FA5151;">待确认</a>';
			}
			if(j.status == 2){
				orderHtml += '<a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_default"  style="color:#000;">已完成</a>';
			}
			orderHtml += '</div>';
			orderHtml += '<div class="weui-cells order-block" style="background:#dddddd;display:none;"><div class="weui-cell consignee-info"><div class="weui-cell__bd"><p>'+j.user_name+'</p></div><div class="weui-cell__ft">预约时间:'+j.timepick+'</div></div><div class="weui-cell consignee-info"><div class="weui-cell__bd"><p>'+j.consignee+'</p></div><div class="weui-cell__ft">'+j.mobile+'</div></div>';
			orderHtml += '<div class="weui-cell consignee-info"><div class="weui-cell__bd"><p>'+j.address+'</p></div><div class="weui-cell__ft view-order" attr-orderid="'+j.order_id+'">查看订单</div></div></div>';
		});
	}
	...
	$('.time-table').click(function() {
		var timepick = $(this).attr('attr-timepick');
		if($(this).hasClass('disabled')){
			return false;
		}
		if($(this).hasClass('choosed')){
			//更新将timepick从technican_schedul[date_]中移除
			$(this).removeClass('choosed');
			var mode = 0;
			var index = technican_schedul[date_].indexOf(timepick);
			technican_schedul[date_].splice(index,1);
		}else{
			//更新将timepick增加到technican_schedul[date_]中
			$(this).addClass('choosed');
			var mode = 1;
			if(typeof(technican_schedul[date_]) == 'undefined'){
				technican_schedul[date_] = new Array();
			}
			technican_schedul[date_].push(timepick);
		}
		
		$('#loadingToast').fadeIn(100);
		//ajax更新技师行程
		$.ajax({
			url: "technican.php?act=schedul_update",
			type: "POST",  
			data: {timepick:timepick,mode:mode,datepick:date_},
			dataType:'json',
...
function gethalfHourArray(type) {
	if (type == 'halfHour') {
		return ['08:00','08:30','09:00', '09:30', '10:00', '10:30', '11:00', '11:30', '12:00', '12:30', '13:00', '13:30', '14:00', '14:30',
