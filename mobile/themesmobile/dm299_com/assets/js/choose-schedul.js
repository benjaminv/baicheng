		...
		for (var i = 0; i < halfHourArray.length; i++) {
			...
			if(nowH>parseInt(schedulH) || (nowH == parseInt(schedulH) && parseInt(nowTime[1]) >= parseInt(halfHourArray[i].split(':')[1]))){
				returnHtml += "<div class='time-table disabled' attr-timepick='"+halfHourArray[i]+"'>" +
							"<div>" + halfHourArray[i] + "<span class='font11'>已过期</span>" + "</div>" +
							"</div>";
			}else{
				var className = "";
				var orderSpan = "";
				//如果该点行程已预定,则增加disabled标签,
				if(typeof(schedul_list[supplier][date_]) != 'undefined'  ){
					var index = $.inArray(halfHourArray[i],schedul_list[supplier][date_]);
					if(index >= 0){
						className = "disabled";
						orderSpan = "<span class='font11'>暂不可约</span>";
					}
				}
				returnHtml += "<div class='time-table "+className+" ' attr-time='"+halfHourArray[i]+"' attr-timepick='"+halfHourArray[i]+"'>" +
