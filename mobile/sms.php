...
if ($_REQUEST['step'] == 'getverifycode')
{
    ...
    $content = '您好，您的验证码'.$verifycode.';【联美美业】';
	/* 发送注册手机短信验证 */
...
if ($_REQUEST['step'] == 'weixin_login')
{
	require_once (ROOT_PATH . 'sms/sms.php');//引入最新的阿里云短信接口 2019/6/19 yhy
    //require(dirname(__FILE__) . '/send.php');
	//require(dirname(__FILE__) . '/includes/lib_sms.php');
	/* 是否开启手机短信验证 */
	if($_CFG['sms_register'] == '0') {
	...
	/* 提交的手机号是否已经注册帐号 */
    //$sql = "SELECT COUNT(user_id) FROM " . $ecs->table('users') ." WHERE mobile_phone = '$mobile'";
	$sql = "SELECT COUNT(a.uid) FROM ".$ecs->table('weixin_user')." a LEFT JOIN ".$ecs->table('users')." b ON a.ecuid=b.user_id WHERE b.mobile_phone = '$mobile'";
	...
	$ret = sendSMS($mobile,$mobile_code,$moban);
	/* 发送注册手机短信验证 */
	if($ret){
		$db->query("delete from ".$ecs->table('verifycode')." where mobile='$mobile'");
		//插入获取验证码数据记录
