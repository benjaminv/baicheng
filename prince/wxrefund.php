 ...
 */
require_once $_SERVER['DOCUMENT_ROOT'] ."/vender/wxpay/lib/WxPay.Api.php";
require_once $_SERVER['DOCUMENT_ROOT'] ."/vender/wxpay/lib/WxPay.Data.php";
require_once $_SERVER['DOCUMENT_ROOT'] ."/vender/wxpay/lib/WxPay.Config.php";
...
function do_wx_refund($order_id,$order_sn,$money_paid,$money_refund){
    ...
    if(isset($order_sn) && $order_sn != ""){
        ...
        wx_refund_log('wxpay refund return:',$return);
        wx_refund_log('+++++++:','end+++++++');
...
function wx_refund_log($key,$content){

    refund_log($key,$content,'weixin');
}

function refund_log($key,$content,$type = 'common'){
//    echo ROOT_PATH.'logs/weixin/pt_refund.log';//die;
    if($type == 'weixin'){
        $dir = 'weixin';
    }elseif($type == 'alipay') {
        $dir = 'alipay';
    ...
    $date = date('Y-m-d');
    $fileName = ROOT_PATH.'logs/'.$dir.'/pt_refund_'.$date.'.log';
