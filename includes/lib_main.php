...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email,u.user_rank, u.user_name, u.user_money, u.pay_points,u.parent_id,u.supplierId,(select supplier_id from '.$GLOBALS['ecs']->table('supplier').' where status=1 And user_id=u.user_id) as u_supplier_id'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function get_affiliate_supplier_id($uid=0){
    ...
    print_r($uinfo);
    if($uinfo){
        //判断如果上级是店铺用户那直接使用此上级的店铺并且上级设置为平台
    ...
    return [$tmp_supplier_id,$tmp_parent_id];
}
