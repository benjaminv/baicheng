...
class weixin {
	...
	//function get_code($order, $payment) {
		...
		//if(! isset($_GET['payopenid']) && !isset($_SESSION['payopenid'])){
			//$url = LIANMEI_EDUCATION.'/app/index.php?i=1&c=auth&a=wxpay&callback='.$selfUrl;
			//Header ( "Location: $url" );exit;
		//}else{
		...
		//$jsApi = new JsApi_pub ();
		//if(BORROW == 1){
			//define ( APPID, BOR_APPID); // appid
			//define ( APPSECRET, BOR_APPSECRET); // appSecret
			//define ( MCHID, BOR_MCHID);
		...
		//define ( NOTIFY_URL, $return_url ); // 成功回调url
		////支付使用的openid从聚亿美进行调用 edit by yhy 2018/12/17 星期一
