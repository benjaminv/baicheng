...
if ($_REQUEST['act']== 'list')
{
    ...
    foreach ($playerdb as $key => $val)
    {
        ...
        // 判断图片的有限时间是否过期
        if(( $val['end'] == 0) || $val['end'] <= local_date('Y-m-d H:i', gmtime())){

            $playerdb[$key]['limit'] = 1; 
           
        }else{

            $playerdb[$key]['limit'] = 0;
        }
    }
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    {   
        ...
        if( strtotime($_POST['url_begin']) > strtotime($_POST['url_end'])){

            $links[] = array('text' => $_LANG['add_new'], 'href' => 'flashplay.php?act=add');
            sys_msg('请添加有效正确的时间', 0, $links);
        }

        if( $_POST['url_begin'] < local_date('Y-m-d H:i', gmtime())){

            $links[] = array('text' => $_LANG['add_new'], 'href' => 'flashplay.php?act=add');
            sys_msg('请增加开始的时间', 0, $links);
        }


        if (empty($_POST['url_begin']) || empty($_POST['url_end']))
        {
            $links[] = array('text' => $_LANG['add_new'], 'href' => 'flashplay.php?act=add');
            sys_msg('请添加图片的有效时间', 0, $links);
        }
...
function get_flash_xml()
{
    ...
    if (file_exists(ROOT_PATH . DATA_DIR . '/'.$flash_file))
    {
        ...
        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"\sbegin="([^"]*)"\s end="([^"]+)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/'.$flash_file), $t, PREG_SET_ORDER))
        {
...
function put_flash_xml($flashdb)
{
    ...
    if (!empty($flashdb))
    {
        ...
        foreach ($flashdb as $key => $val)
        {
            $xml .= '<item item_url="' . $val['src'] . '" link="' . $val['url'] . '" text="' . $val['text'] . '" sort="' . $val['sort'] . '" begin="'.$val['begin'].'"  end="'.$val['end'].'"/>';
        }
        $xml .= '</bcaster>';
