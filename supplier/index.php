...
//    $today['money'] = $db->GetOne('SELECT SUM(money_paid) AS money FROM ' .$ecs->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.' AND supplier_id='.$_SESSION['supplier_id'].order_query_sql('finished'));
	...
	$sql = 'SELECT user_id FROM '.$ecs->table('supplier').' WHERE supplier_id='.$_SESSION['supplier_id'];
    $supplier_uid = $db->getOne($sql);

	$qrcode_url = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/supplier_qrcode.php?u='.$supplier_uid.'&suppId='.$_SESSION['supplier_id'];
    $smarty->assign('qrcode_url',$qrcode_url);
