...
function create_shop_settiongs()
{
    ...
    $supplier = $db->getRow('select * from '.$ecs->table('supplier').' where supplier_id="'.$_SESSION['supplier_id'].'"');
	...
	if($num>0){
		...
		$insql = "INSERT INTO ". $ecs->table('supplier_shop_config') ." (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES
				...
				(103, 1, 'shop_desc', 'text', '', '', '".$supplier['description']."', 1, ".$_SESSION['supplier_id']."),
				...
				(119, 1, 'shop_logo', 'file', '', 'data/supplier', '".$supplier['logo']."', 1, ".$_SESSION['supplier_id']."),
