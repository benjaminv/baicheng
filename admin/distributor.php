...
//        $user['formated_frozen_money'] = price_format(0);
    ...
    $supplierlist=get_supplier_list();
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if(!$user){sys_msg('该分销商不存在，刷新后再试！',0,$link); }

    //判断如果用户是店铺直属会员，那么就可修改所属店铺，并且它的下级所属店铺也会修改
    if($user['parent_id']==0||($user['supplierId']!=0&&$user['supplierId']==$user['parent_supplierId'])){
        if($supplierId!=0){
            if($db->getOne("SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('supplier')." WHERE status=1 And supplier_id='$supplierId'")==0){
                sys_msg("所属店铺不存在，请重新选择！",0,$link);
            }
            $con.=",`supplierId`='$supplierId'";
        }
    }else{
        $supplierId=0;
    }
	...
	if($parent_id == 0) {
        ...
        if($parent_id!=$user['parent_id']){
            ...
            if($affiliate_supplier&&$affiliate_supplier['config']){
                $supplierId=$affiliate_supplier['config']['default_reg_supplier_id'];
                $con.=",`supplierId`='".$affiliate_supplier['config']['default_reg_supplier_id']."'";
            }
        }
	}
	elseif($parent_id > 0)
    ...
    //更新下级所属店铺 edit yhy 2019/8/8上级店铺更改不影响下级会员
    //if($supplierId!=0){
        //$sql="UPDATE ".$ecs->table('users')." SET supplierId='$supplierId' where parent_id='$user[user_id]'";
        //$db->query($sql);
    //}
...
elseif ($_REQUEST['act'] == 'remove_parent')
{
    ...
    $sql = "UPDATE " . $ecs->table('users') . " SET parent_id = 0,supplierId='$supplierId' WHERE user_id = '" . $_GET['id'] . "'";
    //print_r($sql);die();
    $db->query($sql);
...
elseif($_REQUEST['act']=='search_supplier'){
    $keys = json_str_iconv(trim($_GET['keys']));
    $sql = "SELECT supplier_id, supplier_name FROM " . $ecs->table('supplier') .
        " WHERE status=1 And supplier_name LIKE '%" . mysql_like_quote($keys) . "%' OR supplier_id LIKE '%" . mysql_like_quote($keys) . "%'";
    $row = $db->getAll($sql);
...
function user_list(){
    ...
    if ($result === false){
		...
		$sql = "SELECT user_id, user_name, email, is_validated, validated,status,user_money, frozen_money, rank_points, pay_points, reg_time,is_fenxiao,parent_id,(select supplier_name from ".$GLOBALS['ecs']->table('supplier')." where supplier_id=u.supplierId) as supplier_name ".
...
{
    $sql = "SELECT u.*,(select supplierId from ".$GLOBALS['ecs']->table('users')." where user_id=u.parent_id) as parent_supplierId FROM " . $GLOBALS['ecs']->table('users') . " as u WHERE u.user_name = '$username'";
