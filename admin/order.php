...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
		...
		log_supplier_commission($order_id);
		schedul_change($order_id);
		...
		//订单中没有实体商品的话，更改订单状态为已收货已完成,并发放提成,发放积分
		//if(!exist_real_goods($order_id)){
				//$sql = 'update ecs_order_info set order_status=5,shipping_status=0,pay_status=2,is_separate=0 where order_id='.$order_id;
				//$GLOBALS['db']->query($sql);
				//distrib_separate($order_id);
				//supplier_separate_by_orderid($order_id);
				//发货后发放积分
				//$order = $GLOBALS['db']->getRow("SELECT user_id,order_sn,extension_id,extension_code,extension_id,goods_amount,order_id FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id=".$order_id);
				//$integral = integral_to_give($order);
				//log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), '订单:'.$order['order_sn'].'赠送的积分');
		//}
...
elseif($_REQUEST['act'] == 'get_express'){
	require_once(ROOT_PATH . 'includes/cls_json.php');
	$json = new JSON;
	require_once(ROOT_PATH . 'mobile/kuaidi/kuaidi.php');
	$kuaidi = new Express($_CFG['kuaidi100_api'],$_CFG['kuaidi100_customer']);
	$res = $kuaidi->getorder($_REQUEST['com'],$_REQUEST['nu']);
	echo $json->encode($res['data']);
}
	...
	if(ODOO_BC_ERP){
			...
			$queue_data = array('queue_type'=>1,'queue_param'=>serialize(array('ordersn'=>$order_sn)),'operate_status'=>0,'create_time'=>gmtime());
			$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('queue'), $queue_data, 'INSERT', '', 'SILENT');
			sys_msg("订单已进入队列", 0, $links);exit;
	}else{
		$links[] = array('text' => '订单列表', 'href' => 'order.php?act=list');
