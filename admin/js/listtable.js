 ...
 * 创建一个可编辑区
 */
listTable.edit = function(obj, act, id,type,url_type,did)
{
  ...
  txt.onblur = function(e)
  {
    ...
    {
      if(url_type==1){
        res = Ajax.call(listTable.url, "act="+act+"&val=" + encodeURIComponent(Utils.trim(txt.value)) + "&id=" +id+"&type="+type+"&did="+did, null, "POST", "JSON", false);
      }else{
        res = Ajax.call(listTable.url, "act="+act+"&val=" + encodeURIComponent(Utils.trim(txt.value)) + "&id=" +id+"&type="+type, null, "POST", "JSON", false);
      }
