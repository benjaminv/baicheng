...
function create_html_editor($input_name, $input_value = '',$width=1000)
{
    ...
    <script type="text/javascript" charset="utf-8" src="../includes/ueditor/ueditor.all.js?v=1.004"></script>
...
function get_goods_list($filter)
{
    ...
    $supplier_id = intval($filter->supplier_id);

    $where = get_where_sql($filter); // 取得过滤条件

    $where .= " AND g.is_on_sale = 1 and g.supplier_id=".$supplier_id;
    /* 取得数据 */
    $sql = 'SELECT goods_id, goods_name, shop_price,supplier_id,market_price '.
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $where;

    $row = $GLOBALS['db']->getAll($sql);
	// foreach($row as $k=>$v){
	// 	$row[$k]['supplier_name'] = $GLOBALS['db']->getOne( "SELECT IFNULL((SELECT supplier_name FROM ". $GLOBALS['ecs']->table('supplier')." WHERE supplier_id=".$v['supplier_id']."),'平台自营')");
	// }
    return $row;
}
