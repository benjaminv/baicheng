...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    if (isset($_POST['ok']))
    {
		...
		if(ODOO_ERP){
			//edit yhy 
			$res = $GLOBALS['odooErpObj']->syncRefundOrderByOrdersn($back_id,0,$order['back_type']);
		}
...
elseif ($_REQUEST['act'] == 'operate_refund')
{
	...
	//查询运费退款到余额还是现金
	if($_REQUEST['refund_shipping'] && intval($_REQUEST['refund_shipping_fee'])>0){
		//查询订单支付的现金和余额
		$order_info = $GLOBALS['db']->getRow("SELECT money_paid,surplus,shipping_fee FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = ".$order['order_id']);
		//查询退款单已经退款的现金和余额
		$back_money = $GLOBALS['db']->getRow("SELECT SUM(`refund_money_1`) as money_paid,SUM(`refund_money_2`) as surplus,SUM(`refund_money_3`) as integral,SUM(`shipping_fee`) as shipping_fee FROM ".$GLOBALS['ecs']->table('back_order')." WHERE order_id=".$order['order_id']);
		$left_shipping = $order_info['shipping_fee'] - $back_money['shipping_fee'];
		if($_REQUEST['refund_shipping_fee']>$left_shipping){
			$links[] = array('text' => '返回退款/退货及维修详情', 'href' => 'back.php?act=back_info&back_id=' . $back_id);
			sys_msg("剩余可退款运费".$left_shipping."!", 0, $links);
			exit;
		}
		

		$left_surplus = $order_info['surplus']-$back_money['surplus'];
		if($left_surplus>0){
			$order['refund_money_2'] += min(intval($_REQUEST['refund_shipping_fee']),$left_surplus);
			$_REQUEST['refund_shipping_fee'] -= min(intval($_REQUEST['refund_shipping_fee']),$left_surplus);
		}
		$left_money_paid = $order_info['money_paid']-$back_money['money_paid'];
		if($left_money_paid>0){
			$order['refund_money_1'] += min(intval($_REQUEST['refund_shipping_fee']),$left_money_paid);
		}
	}
	//月梦网络添加
    ...
    if($order['refund_money_1']>0){
		$refund_money_1 = $order['refund_money_1'];
		//微信支付金额和支付宝支付金额的退款
		if($_REQUEST['refund_shipping']){
			$refund_money_1 = $order['refund_money_1'] + $_REQUEST['refund_shipping_fee'];
		}
		...
		$pay_code = $GLOBALS['db']->getOne("SELECT pay_code FROM " . $GLOBALS['ecs']->table('payment') ." WHERE pay_id = ".$order_row['pay_id']);
		if($pay_code == 'weixin'){
			$order['log_id'] = $row['log_id'];
			$order_sn=$order['log_id']."-".($row['order_amount'] * 100);
			$money_paid=$row['order_amount'];
			$money_refund=$refund_money_1;
	...
	//将余额和积分退还到用户账户,并更改订单和退款单状态
	$desc_back = "订单". $order['order_id'] .'退款';
	if($order['refund_money_2']>0){
		log_account_change($order['user_id'], $order['refund_money_2'],0,0,0, $desc_back );
	}
	if($order['refund_money_3']>0){
		log_account_change($order['user_id'], 0,0,$order['refund_money_3'],0, $desc_back );
	}
	...
	$shipping_sql = "";
	if($_REQUEST['refund_shipping'] && intval($_REQUEST['refund_shipping_fee'])>0){
		$shipping_sql = ",shipping_fee=".intval($_REQUEST['refund_shipping_fee']);
	}
	$sql2 = "update ". $ecs->table('back_order') ." set  status_refund='$status_refund',refund_type='$_REQUEST[refund_type]', refund_desc='$refund_desc'".$shipping_sql." where back_id='$back_id' ";
	$db->query($sql2);
...
elseif($_REQUEST['act'] == 'sysnc_erp'){
	...
	if(!$back_id){
		...
		if(ODOO_ERP){
			$res = $GLOBALS['odooErpObj']->syncRefundOrderByOrdersn($back_id,1,$backInfo['back_type']);
...
function back_list()
{
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $is_robot = $value['is_robot'] ? '自动' : '';

        if($value['is_robot']){
            $row[$key]['status_back_val'] = $row[$key]['status_back_val'].'('.$is_robot.')';
        }
