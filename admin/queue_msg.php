...
elseif($_REQUEST['act'] == 'done')
{
	...
	//查询最近的一个未执行订单
	if(ODOO_BC_ERP){
		$sql = 'SELECT * FROM '. $ecs->table('queue') .' WHERE  id='.intval($id);
		...
		if(!isset($info) || empty($info)){
			...
			$oddo_erp_db = $db->getOne("SELECT b.databases_name FROM ".$ecs->table('order_info')." a LEFT JOIN ".$ecs->table('supplier')." b ON a.from_supplier_id=b.supplier_id WHERE a.order_sn='".$ordersn."'");
			if(!$oddo_erp_db){
				$link[] = array('text' => $_LANG['back_queue_list'], 'href'=>'queue_msg.php?act=list');
			...
			if(isset($res['SuccessCode']) && $res['SuccessCode'] == 1){
				...
				$queue_update_data['operate_result'] = '同步失败';
			}
