...
if ($_REQUEST['act'] == 'list')
{
    $logdb = get_affiliate_ck();
     ...
     // 处理info为空的数据，防止前端显示空白
    foreach ( $logdb['logdb'] as $key =>$val) {

        if(!$val['info']){ // 为空
            
            unset($logdb['logdb'][$key]);
        } 
    }
...
function get_affiliate_ck()
{
	...
	else
	{
		...
		"(select o.order_id,o.user_id,o.add_time,o.order_status,o.shipping_status,".
	...
	if($GLOBALS['_CFG_MOBILE']['distrib_type'] == 0)
	{
		$sql = "SELECT order_sn,is_separate,order_id,o.user_id,add_time,order_status,shipping_status,o.supplier_id,u.user_name FROM ".$GLOBALS['ecs']->table('order_info')." AS o," . $GLOBALS['ecs']->table('users') . " as u WHERE o.pay_status = 2 AND o.user_id = u.user_id $sqladd ORDER BY order_id DESC LIMIT " . $filter['start'] . ",$filter[page_size]";
	}
	else
	{
		$sql = "select order_sn,is_separate,order_id,user_id,add_time,
		order_status,shipping_status,supplier_id,user_name from " .
		"(select o.order_id,o.order_sn,o.user_id,o.add_time,o.order_status,o.shipping_status,".
    ...
    {
		$info = get_all_affiliate_log1($rt['order_id']);
		...
		$rt['supplier'] = get_supplier1($rt['supplier_id']);
		$logdb[] = $rt;
	}
...
function write_affiliate_log1($oid, $uid, $username, $money, $separate_by,$change_desc,$grade = 0,$separate_type = 1)
{
...
//分成后，推送到各个上级分销商微信
function push_user_msg1($ecuid,$order_sn,$split_money){
	$type = 1;
...
//根据订单号获取分成日志信息
function get_all_affiliate_log1($order_id)
{
	...
	foreach($list as $val)
	{
		if($val['money']>0){
			...
			if($val['level_point']>0){

				$str .= sprintf('用户ID %s ( %s ), 获得的等级积分： %s积分', $val['user_id'], $val['user_name'], $val['level_point'])."<br />";
				$arr['log_id'] = $val['log_id'];
				$arr['separate_type'] = $val['separate_type'];
			}else{
				$str .= sprintf('用户ID %s ( %s ), 获得的销售积分： %s积分', $val['user_id'], $val['user_name'], $val['sale_point'])."<br />";
				$arr['log_id'] = $val['log_id'];
				$arr['separate_type'] = $val['separate_type'];
			}
