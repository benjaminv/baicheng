...
if ($_REQUEST['act']=='reply')
{
    ...
    //获取追加评论
    $comment_added=$db->getRow("SELECT * FROM ".$ecs->table('comment_added')." WHERE comment_id=".$comment_info['comment_id']);
    if($comment_added){
        $comment_added['content']  = str_replace('\r\n', '<br />', htmlspecialchars($comment_added['content']));
        $comment_added['content']  = nl2br(str_replace('\n', '<br />', $comment_added['content']));
        $comment_added['add_time'] = local_date($_CFG['time_format'], $comment_added['add_time']);
    }
    ...
    $smarty->assign('comment_added',$comment_added); //追加评论信息
...
//--By Freely
/*------------------------------------------------------ */
elseif($_REQUEST['act']=='addedcheck'){
    $status=$_REQUEST['check']=='allow'?1:0;
    $comment_added=$db->getRow("SELECT * FROM ".$ecs->table('comment_added')." WHERE comment_added_id='$_REQUEST[id]'");
    if($comment_added){
        $sql = "UPDATE " .$ecs->table('comment_added'). " SET status = $status WHERE comment_added_id = '$comment_added[comment_added_id]'";
        $db->query($sql);
        /* 清除缓存 */
        clear_cache_files();
        ecs_header("Location: comment_manage.php?act=reply&id=$comment_added[comment_id]\n");
        exit;
    }
}
