    ...
    if ($account['is_paid'] == 0)
    {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1')
        {
            ...
            if($account['pay_typ']==0){
                ...
                //使用支付宝付款
                include_once(dirname ( __FILE__ ).'/includes/payment/alipay.php');
                //require("includes/payment/alipay.php");
                $pay_obj    = new cash_alipay();
                $rdata = $pay_obj->payToUser('ua_'.$account['id'],$account['pay_number'],$account['pay_name'],'提现成功',$fmt_amount);
                if($rdata['status']!=1){
