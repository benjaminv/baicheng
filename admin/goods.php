...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy'){
    ...
    $goods['level_arr'] = explode(',',$goods['level']);
	...
	//卡项商品的标志
	if(isset($_REQUEST['goods_item']) && $_REQUEST['goods_item'] == 1){
		$smarty->assign('item_id', intval($_REQUEST['item_id']));
	}
	$smarty->assign('goo', $poster_list);
    /* 显示商品信息页面 */
...

{
	...
	$is_service = !empty($_POST['is_service']) ? intval($_POST['is_service']) : 0;
	$poster_id = !empty($_POST['poster_id']) ? intval($_POST['poster_id']) : 0;
	$service_group = !empty($_POST['service_group']) ? trim($_POST['service_group']) : "";
...

    {
...

        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, ghost_count, goods_sn,morning_market_price,is_morning_market, " .
                    ...
                    "cat_id, brand_id, shop_price, market_price,is_service,item_id,poster_id,service_group, is_promote, zhekou, promote_price, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, suppliers_id, cost_price, points_payment_amount,`level`)" .

                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$ghost_count', '$goods_sn','$morning_market_price','$is_morning_market', '$catgory_id', " .

                    "'$brand_id', '$shop_price', '$market_price','$is_service','$item_id','$poster_id','$service_group', '$is_promote', '$zhekou', '$promote_price', ".
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$suppliers_id', '$cost_price','$points_payment_amount','$level')";
...

        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, ghost_count, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, cost_price,points_payment_amount,`level`)" .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$ghost_count', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral', '$cost_price','$points_payment_amount','$level')";
   ...
   /*编辑商品分成金额成为分销商品*/
...

    {
...

        {
			...
			/* reformat_image_name会将原图删除，导致goods_img图片丢失 edit yhy 2019/7/25 此处可以需要再详细查看*/
	...
	if(intval($_POST['item_id'])){
		$link2[] = array('href' => 'card_item.php?act=goods&item_id=' . intval($_POST['item_id']), 'text' => '卡项商品');
		sys_msg($is_insert ? $_LANG['add_goods_ok'] : $_LANG['edit_goods_ok'], 0, $link2);
	}else{
...
elseif($_REQUEST['act'] == 'sysnc_erp'){
	...
	if(!$goodsid){
		...
		if(ODOO_ERP){
			...
			$res = $GLOBALS['odooErpObj']->syncGoodsAndProductToErpByGoodsid($goodsid);
