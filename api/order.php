...
$orderActionList = array("update",'delivery','deposit','refund','technican');
if($auth != trim($_POST['auth'])){
...
function api_action_technican(){
    ...
    if($type == 'add'){ // 添加技师
		...
		if(!$array['supplier_id']){
			...
			echo json_encode(array("result"=>"fail","code"=>303,"msg"=>"店铺id不存在"));exit;
		}
		if(!$array['technican_name']){
			
			echo json_encode(array("result"=>"fail","code"=>303,"msg"=>"技师名称不存在"));exit;
		}
		if(!$array['technican_sn']){
			
			echo json_encode(array("result"=>"fail","code"=>303,"msg"=>"技师编号不存在"));exit;
		}

	    $data = [
	    	'supplier_id'=>$array['supplier_id'],       // 技师所在店铺
	    	'technican_name'=>$array['technican_name'], //技师名称
	    	'technican_sn'=>$array['technican_sn'],     // 技师编号
	    	'add_time'=>gmtime(),      // 添加时间
	    	'update_time'=>gmtime(),      //修改时间
	    ];

	    $rel = $db->autoExecute($ecs->table('technican'), $data, 'INSERT', '', 'SILENT');

	    if($rel){
	    	//增加同步失败日志
			echo json_encode(array("result"=>"success","code"=>200,"msg"=>'添加技师信息成功'));exit;

	    }else{

	    	echo json_encode(array("result"=>"fail","code"=>500,"msg"=>$rel));exit;
	    ...
	    $rel = $db->autoExecute($ecs->table('technican'), $data,  'UPDATE', 'technican_sn = "'.$array['technican_sn'].'" and supplier_id="'.$array['supplier_id'].'"');
        ...
        $rel = $db->query('delete from ecs_technican where supplier_id="'.$array['supplier_id'].'" and technican_sn="'.$array['technican_sn'].'"');
    	...
    	echo json_encode(array("result"=>"fail","code"=>303,"msg"=>'操作不存在'));exit;
    }	
}